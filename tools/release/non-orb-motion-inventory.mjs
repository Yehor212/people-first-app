...
const MODULE_EXTENSIONS = [".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".css", ".scss", ".json", ".frag", ".vert", ".glsl"];
const TEST_PATH = /(?:^|\/)(?:__tests__|test|tests|e2e|fixtures?)(?:\/|$)|\.(?:test|spec)\.[cm]?[jt]sx?$/;
...
const CATEGORY_RULES = [
  ...
  ["css-animation", /\banimation(?:-name|-duration|-timing-function|-iteration-count|-delay|-fill-mode|-play-state)?\s*:|\banimate-[a-z0-9_[\]-]+/i],
  ["css-transition", /\btransition(?:-property|-duration|-timing-function|-delay)?\s*:|\btransition(?:-[a-z0-9_[\]-]+)?\b/i],
  ["framer-motion", /framer-motion|motion\/react|<motion\.|\bAnimatePresence\b|\b(?:initial|animate|exit|whileHover|whileTap|whileFocus|layout)\s*=/i],
  ...
  ["canvas-webgl", /<canvas\b|\.getContext\s*\(|\bWebGL(?:2)?RenderingContext\b|\bwebgl2?\b|\bOffscreenCanvas\b|\bgl_(?:Position|FragColor)\b|\bu_time\b/i],
  ["particles-confetti", /\bparticles?\b|\bconfetti\b|\bdust\b|\bmotes?\b/i],
  ["lottie-tgs-video-gif", /\blottie\b|\.tgs\b|\.gif\b|<video\b|\.mp4\b|\.webm\b|requestVideoFrameCallback/i],
  ["audio-synchronized", /\bAudioContext\b|\bwebkitAudioContext\b|\bnew\s+Audio\s*\(|<audio\b|requestVideoFrameCallback/i],
  ["native-route-splash-transition", /\bSplashScreen\b|windowSplashScreenAnimatedIcon|UIView\.animate|ObjectAnimator|ValueAnimator|overridePendingTransition|DialogMotionContent|SheetMotionContent|\btoast\b|\bpageTransition\b|\brouteTransition\b/i],
];
const DIRECT_CATEGORIES = new Set(CATEGORY_RULES.map((item) => item[0]).filter((item) => item !== "timer"));
...
function walk(root, directory) {
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name))) {
    if (["node_modules", "dist", "build", ".git", "coverage", "output", "target", "DerivedData"].includes(entry.name)) continue;
...
function allSourceFiles(root) {
  const roots = ["src", "public", "android/app/src/main", "ios/App/App", "ios/App/CapApp-SPM/Sources", "src-tauri/src"];
  return roots
...
function imports(root, path, text) {
  if (/\.(?:css|scss)$/.test(path)) {
    return [...text.matchAll(/@import\s+["']([^"']+)["']/g)].map((match) => ({ specifier: match[1], kind: "css-import" }));
  }
  ...
  function visit(node) {
    if (ts.isImportDeclaration(node) && ts.isStringLiteral(node.moduleSpecifier)) {
      ...
      const allNamedType = clause && !clause.name && named && ts.isNamedImports(named) && named.elements.length > 0 && named.elements.every((item) => item.isTypeOnly);
      if (!clause || (!clause.isTypeOnly && !allNamedType)) found.push({ specifier: node.moduleSpecifier.text, kind: "static-import" });
    ...
    } else if (ts.isNewExpression(node) && node.expression.getText(file) === "URL" && node.arguments && node.arguments.length > 0 && ts.isStringLiteralLike(node.arguments[0])) {
...
function importGraph(root) {
  ...
  while (queue.length) {
    ...
    if (!path || reachable.has(path) || !existsSync(absolute) || TEST_PATH.test(path) || DEV_PATH.test(path)) continue;
    reachable.add(path);
...
function functionRanges(path, text) {
  ...
  function nameFor(node) {
    ...
    if (ts.isCallExpression(node.parent) && ts.isVariableDeclaration(node.parent.parent)) return node.parent.parent.name.getText(file);
    return "anonymous";
  }
  function visit(node) {
...
function cssSymbol(lines, lineIndex, category) {
  ...
  for (let index = lineIndex; index >= 0 && index >= lineIndex - 30; index -= 1) {
    ...
    if (candidate.endsWith("{") && !candidate.startsWith("@")) return "css " + candidate.slice(0, -1).trim().slice(0, 120);
  }
...
function routeFor(path, symbol) {
  ...
  if (/diary|journal|gratitude|thought|burn/.test(value)) return { route: "diary", surface: symbol };
...
function visibleMeaning(symbol, path) {
  ...
  if (/gratitude|bloom/.test(value)) return { value: "gratitude save/planting completion feedback", status: "SOURCE_INFERRED" };
  if (/burnthought|letgo|release|dust/.test(value)) return { value: "transient thought release feedback", status: "SOURCE_INFERRED" };
  if (/splash|loader|loading|spinner|skeleton/.test(value)) return { value: "pending or startup progress", status: "SOURCE_INFERRED" };
  if (/toast|banner|notification/.test(value)) return { value: "status notification entry/exit", status: "SOURCE_INFERRED" };
  if (/modal|dialog|sheet|drawer|popover|menu/.test(value)) return { value: "layer entry, exit or state change", status: "SOURCE_INFERRED" };
  if (/celebr|confetti|achievement|success/.test(value)) return { value: "completion or celebration feedback", status: "SOURCE_INFERRED" };
  if (/navigation|route|page/.test(value)) return { value: "navigation or route continuity", status: "SOURCE_INFERRED" };
...
function lifecycleFor(ownerText, categories) {
  ...
  return {
    timer: resource([/\bsetTimeout\s*\(/, /\bsetInterval\s*\(/], [/\bclearTimeout\s*\(/, /\bclearInterval\s*\(/]),
    ...
    canvas: resource([/<canvas\b/, /\.getContext\s*\(/, /\bOffscreenCanvas\b/], [/\bremove\s*\(/, /\bdispose\s*\(/, /\bclose\s*\(/, /\bnull\b/]),
    audio: resource([/\bAudioContext\b/, /\bnew\s+Audio\s*\(/, /<audio\b/], [/\bpause\s*\(/, /\bclose\s*\(/, /\bdisconnect\s*\(/]),
    categories,
...
function reducedMotionFor(root, ownerText, categories) {
  const local = /\buseShouldAnimate\b|\bshouldAnimate\s*\(|prefers-reduced-motion|reducedMotion|reduce-motion/.test(ownerText);
  ...
  const sharedCss = existsSync(cssPath) && /body\.reduce-motion|prefers-reduced-motion/.test(readRooted(root, "src/index.css"));
  if (local) return { sourceStatus: "PASS_LOCAL_GATE_DETECTED", behavior: "local effective-motion source branch detected", runtime: "UNVERIFIED" };
  if (categories.includes("framer-motion") && sharedMotion) return { sourceStatus: "PASS_SHARED_MOTIONCONFIG_SOURCE", behavior: "inherits app MotionConfig source gate", runtime: "UNVERIFIED" };
  if (categories.some((item) => item.startsWith("css-")) && sharedCss) return { sourceStatus: "PASS_SHARED_CSS_KILL_SWITCH_SOURCE", behavior: "shared CSS reduced-motion source gate exists", runtime: "UNVERIFIED" };
  return { sourceStatus: "UNVERIFIED", behavior: "no owner-local reduced-motion branch detected", runtime: "UNVERIFIED" };
}

function exitsFor(ownerText, symbol) {
  const takeover = /modal|dialog|sheet|drawer|popover|overlay|toast/i.test(symbol);
  return {
    visible: sourceFact(ownerText, [/\bClose\b/, /\bCancel\b/, /\bDismiss\b/, /\bonClose\b/, /\bonOpenChange\b/], "PASS_SOURCE_EXIT_DETECTED", takeover ? "visible exit not detected" : "not a takeover"),
    androidBack: takeover
      ? sourceFact(ownerText, [/\buseBackHandler\b/, /\bregisterModalCloseCallback\b/, /\bonBack\b/], "PASS_SOURCE_BACK_OWNER_DETECTED", "Android Back owner not detected")
      : { sourceStatus: "N/A_INLINE_NON_TAKEOVER", runtimeStatus: "UNVERIFIED", evidencePatterns: [] },
    escape: takeover
      ? sourceFact(ownerText, [/\bEscape\b/, /\bonEscapeKeyDown\b/, /\buseModalKeyboard\b/], "PASS_SOURCE_ESCAPE_DETECTED", "Escape owner not detected")
      : { sourceStatus: "N/A_INLINE_NON_TAKEOVER", runtimeStatus: "UNVERIFIED", evidencePatterns: [] },
    focus: takeover
      ? sourceFact(ownerText, [/\bfocus\s*\(/, /\bautoFocus\b/, /\bonCloseAutoFocus\b/, /\bFocusScope\b/], "PASS_SOURCE_FOCUS_OWNER_DETECTED", "focus ownership not detected")
      : { sourceStatus: "N/A_INLINE_NON_TAKEOVER", runtimeStatus: "UNVERIFIED", evidencePatterns: [] },
  };
}

function platformApplicability(path, route) {
  const native = path.startsWith("android/") ? "android" : path.startsWith("ios/") ? "ios" : path.startsWith("src-tauri/") ? "desktop" : null;
...
function privacyFor(path, symbol, ownerText) {
  const sensitive = /journal|diary|thought|gratitude|mood|habit|auth|token/i.test(path + " " + symbol + " " + ownerText);
  return {
...
function triggerFor(categories, tokens) {
  if (tokens.some((token) => /\bwhileHover\b|\bwhileTap\b|\bonClick\b/.test(token))) return "direct pointer/keyboard interaction";
  if (tokens.some((token) => /\buseEffect\b/.test(token))) return "component lifecycle or state effect";
...
function rowFor(root, graph, path, text, group, reachability) {
  ...
  const source = {
    ...
    sha256: sha256(text),
    locators: group.occurrences.map((item) => ({ line: item.line, category: item.category, token: item.token })),
  };
  ...
  const gated = /\b(?:FEATURE|ENABLED|shouldShow|isEnabled|designFlag|featureFlag)\b/.test(ownerText);
  const entryPath = reachability === "source-entry" ? importPath(graph, path) : [{ file: path, via: reachability }];
  return {
    id: "t191-" + slug(path) + "-" + slug(group.symbol),
    source,
    routeSurface: surface,
    trigger: triggerFor(categories, source.locators.map((item) => item.token)),
    ...
    reachability: {
      ...
      sourceState: gated ? "gated" : reachability === "source-entry" || reachability.includes("shipped") ? "reachable" : "unknown",
      bundle: {
        status: path.startsWith("android/") || path.startsWith("ios/") || path.startsWith("src-tauri/")
        ...
        proofBoundary: "shared category sentinel plus exact source import path; not a unique per-owner runtime observation",
      },
    },
    classification: {
      state: gated ? "gated" : "reachable",
      confidence: categories.some((item) => DIRECT_CATEGORIES.has(item)) ? "CONFIRMED_SOURCE_MOTION" : "AMBIGUOUS",
    },
    ...
    localization: {
      locales: LOCALES,
      rtlSensitivity: /\b(?:left|right|translateX|x:|direction|dir=|rtl:)\b/.test(ownerText) ? "HIGH_DIRECTIONAL_SOURCE" : "REVIEW_REQUIRED",
      runtime: "UNVERIFIED",
    },
...
function exclusionFor(path, text, occurrences, reason) {
  return {
    ...
    source: {
      file: path,
      sha256: sha256(text),
      locators: occurrences.slice(0, 20).map((item) => ({ line: item.line, category: item.category, token: item.token })),
    },
...
export function discoverInventory(root = TOOL_ROOT) {
  ...
  for (const path of files) {
    if (MOTION_ASSET.test(path)) {
      const text = path;
      const occurrence = { category: "lottie-tgs-video-gif", line: 1, token: path, symbol: "asset " + path.split("/").at(-1), start: 1, end: 1 };
      const row = rowFor(root, graph, path, text, { symbol: occurrence.symbol, occurrences: [occurrence] }, "public-shipped-asset");
      motionOwners.push(row);
      candidateFileLedger.push({ file: path, candidateCount: 1, classification: "INVENTORIED_1_OWNER", ownerIds: [row.id], evidence: [path + ":1"] });
    ...
    for (const item of occurrences) oracleLocators.push(path + ":" + item.line + ":" + item.category);
    ...
    if (TEST_PATH.test(path)) {
      ...
      nonProductionExclusions.push(exclusionFor(path, text, occurrences, "dead-unreachable-from-production-entry"));
      classification = "JUSTIFIED_DEAD_UNREACHABLE";
    } else {
      const directGroups = groups.filter((group) => group.occurrences.some((item) => DIRECT_CATEGORIES.has(item.category)));
      if (!directGroups.length) {
        nonProductionExclusions.push(exclusionFor(path, text, occurrences, "ambiguous-timer-visual-role-unproven"));
        ...
        for (const group of directGroups) {
          ...
          if (ORB_FAMILY_FILES.has(path) || /^(?:ValenceOrb|MiniValenceOrb|CompactValenceOrb)$/.test(group.symbol)) {
            const exclusion = {
              ...row,
    ...
    candidateFileLedger.push({
      ...
      evidence: occurrences.slice(0, 20).map((item) => path + ":" + item.line + ":" + item.category),
    });
  }
  ...
  for (const row of [...motionOwners, ...orbExclusions]) {
    for (const category of row.motionCategories) categoryCounts[category] = (categoryCounts[category] || 0) + 1;
  }
  ...
  return {
    ...
    summary: {
      ...
      ambiguousTimerFiles: candidateFileLedger.filter((item) => item.classification === "AMBIGUOUS_TIMER_VISUAL_ROLE_UNPROVEN").length,
      categoryCounts,
...
function validateSource(root, row, errors) {
  ...
  for (const locator of row.source.locators || []) {
    if (!locator.token || !text.includes(locator.token)) errors.push("STALE_SOURCE_LOCATOR " + path + ":" + (locator.line || "unknown"));
  }
}
...
export function validateInventory(root, expected, actual) {
  ...
  for (const row of actual.motionOwners) {
    ...
    if (ORB_FAMILY_FILES.has(row.source && row.source.file)) errors.push("ORB_EXCLUSION_MISLABELED " + row.id);
    for (const path of REQUIRED_ROW_PATHS) if (!hasPath(row, path)) errors.push("MISSING_FIELD " + (row.id || "unknown") + " " + path);
    validateSource(root, row, errors);
  }
  for (const id of expectedIds) if (!actualIds.has(id)) errors.push("MISSING_DISCOVERED_OWNER " + id);
  ...
  for (const row of actual.orbExclusions) {
    ...
    if (row.classification !== "canonical-orb-family") errors.push("ORB_EXCLUSION_MISLABELED " + row.id);
    if (!Array.isArray(row.proof) || row.proof.length < 2) errors.push("ORB_EXCLUSION_WITHOUT_PROOF " + row.id);
    if (!ORB_FAMILY_FILES.has(row.source && row.source.file)) errors.push("ORB_EXCLUSION_PATH_NOT_CANONICAL " + row.id);
  ...
  for (const id of expectedOrbIds) if (!actualOrbIds.has(id)) errors.push("MISSING_ORB_EXCLUSION " + id);
  ...
  if (!Array.isArray(oracleRows)) {
    ...
    for (const row of oracleRows) {
      ...
      for (const id of row.ownerIds) {
        if (!accountedOwnerIds.has(id)) errors.push("SOURCE_ORACLE_UNKNOWN_OWNER " + row.file + " " + id);
      }
    }
  }
  if (actual.summary.motionOwnerRows !== actual.motionOwners.length) errors.push("SUMMARY_OWNER_COUNT_MISMATCH");
  if (actual.summary.orbExclusionRows !== actual.orbExclusions.length) errors.push("SUMMARY_ORB_COUNT_MISMATCH");
  if (Array.isArray(oracleRows) && actual.summary.accountedCandidateFiles !== oracleRows.length) errors.push("SUMMARY_ORACLE_COUNT_MISMATCH");
  if (actual.summary && actual.summary.coveragePercent !== 100) errors.push("SOURCE_ORACLE_COVERAGE " + actual.summary.coveragePercent);
  if (!Array.isArray(actual.summary && actual.summary.validatorFailures) || actual.summary.validatorFailures.length) errors.push("VALIDATOR_FAILURES_NOT_EMPTY");
  if (!errors.length && JSON.stringify(actual) !== JSON.stringify(expected)) errors.push("INVENTORY_DRIFT deterministic generated inventory differs");
  return errors;
}

function bundleFiles(root, directory) {
  const absolute = safeAbsolute(root, directory);
  if (!existsSync(absolute) || !statSync(absolute).isDirectory()) throw new Error("BUNDLE_DIRECTORY_MISSING " + directory);
...
function writeBundleReceipt(root, inventory, bundleDirectory, receiptPath) {
  ...
  const rowProofs = inventory.motionOwners.map((row) => {
    ...
    return {
      id: row.id,
      status: nativeNarrow ? "N/A_NATIVE_SOURCE_ONLY" : found.length ? "PASS_SHARED_CATEGORY_SENTINEL" : "FAIL_NO_BUNDLE_SENTINEL",
  ...
  const receipt = {
    ...
    bundleSha256: sha256(files.map((item) => item.path + "\0" + item.sha256 + "\0" + item.size).join("\n") + "\n"),
    ...
    androidBundleProof: failures.length || searchable.includes(TEST_ONLY_BUNDLE_SENTINEL) ? "FAIL" : "PASS_SOURCE_GRAPH_PLUS_SHARED_SENTINELS",
    externalWrites: 0,
  };
...
try {
  ...
  console.error("FAIL non-orb-motion-inventory SOURCE_DISCOVERY " + (error instanceof Error ? error.message : String(error)));
  process.exit(2);
}
...
if (options.bundle) {
  ...
  try {
    ...
    if (receipt.failures.length) {
      for (const failure of receipt.failures) console.error("FAIL non-orb-motion-inventory BUNDLE " + failure);
      process.exit(1);
    }
    console.log("PASS non-orb-motion-inventory BUNDLE " + receipt.bundleRowsPass + "/" + receipt.bundleRowsTotal + " shared rows; native-source-only=" + receipt.nativeSourceOnlyRows);
    process.exit(0);
  } catch (error) {
    console.error("FAIL non-orb-motion-inventory BUNDLE_PROOF " + (error instanceof Error ? error.message : String(error)));
    process.exit(2);
  }
}
...
if (options.mode === "write") {
  ...
  let actual;
  try {
    actual = options.stdin ? JSON.parse(readFileSync(0, "utf8")) : JSON.parse(readRooted(options.root, DEFAULT_INVENTORY));
  } catch (error) {
    console.error("FAIL non-orb-motion-inventory INVENTORY_READ " + (error instanceof Error ? error.message : String(error)));
    process.exit(1);
  }
  ...
  console.log("PASS non-orb-motion-inventory " + summary(actual) + "; runtime=UNVERIFIED; artistic=UNVERIFIED; release=STOP");
}
