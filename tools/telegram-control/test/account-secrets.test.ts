...
void test("account-secret checks keep missing secrets unverified", () => {
  ...
  assert.deepEqual(checks.map((check) => check.status), ["UNVERIFIED", "UNVERIFIED"]);
...
void test("account-secret validation rejects malformed ids and private keys", () => {
  ...
  assert.deepEqual(validateAccountSecret("GITHUB_APP_ID", "abc"), ["GITHUB_APP_ID must be numeric"]);
