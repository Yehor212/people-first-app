...
void test("activation runner defaults to non-mutating dry-run steps", () => {
  ...
  assert.equal(
    steps.every((step) => !step.mutatesExternalState),
    true
  );
  ...
  assert.equal(
    steps.find((step) => step.id === "worker-deploy")?.displayCommand.endsWith("deploy:dry-run"),
    true
  );
  assert.equal(
  ...
  assert.equal(
    steps.find((step) => step.id === "github-app-manifest")?.mutatesExternalState,
    false
  );
  assert.equal(
...
void test("activation runner apply mode requires explicit mutating flags", () => {
  ...
  assert.equal(
    steps.some((step) => step.mutatesExternalState),
    false
  );
...
void test("activation runner enables selected live steps without printing namespace id", () => {
  const options = parseActivationRunOptions(["--apply", "--kv", "--deploy", "--github-callback"], {
  ...
  assert.match(
    steps.find((step) => step.id === "kv")?.displayCommand ?? "",
    /<CONTROL_STATE_KV_ID>/
  );
  assert.doesNotMatch(
...
void test("activation runner all mode selects every live activation step", () => {
  ...
  assert.equal(
    steps.find((step) => step.id === "kv")?.displayCommand.endsWith("--create --write"),
    true
  );
  assert.equal(
    steps.find((step) => step.id === "cloudflare-account-secrets")?.mutatesExternalState,
    true
  );
  assert.equal(
    steps.find((step) => step.id === "github-app-manifest")?.mutatesExternalState,
  ...
  assert.match(
    ...
    /--github --github-telegram --github-snyk/
  );
  assert.doesNotMatch(
    steps.find((step) => step.id === "github-account-secrets")?.displayCommand ?? "",
    /openai/
...
void test("activation runner installs the Telegram bot token into GitHub secrets explicitly", () => {
  ...
  assert.match(step?.displayCommand ?? "", /--github --github-telegram/);
});
