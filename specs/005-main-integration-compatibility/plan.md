...
### Approved PWA Delivery Follow-up

Owner approval at 15:23 UTC extends T019 to the cold Diary editor. Use `scripts/diary-offline-precache.mjs` to collect the exact emitted chunk containing `src/features/journal/JournalEntryEditor.tsx`, recursively follow static `imports` with a visited set, and include Vite CSS/emitted-asset metadata. Ignore optional dynamic imports; reject missing roots/edges and external dependencies. Content revisions derive from emitted bytes. A Workbox manifest transform merges by URL without mutating existing entries, preserving shell revisions and applying a 512 KiB added-payload budget plus the existing 3 MiB per-file maximum. The five currently missing chunks total 318558 bytes before compression; the limit leaves bounded headroom, not permission for all-JS caching.

Wire the collector and transform only when the existing `pwaEnabled` flag is true. Keep `src/sw.ts`, registration, cleanup/update policy, app lazy imports, storage and visuals unchanged. The transform is chosen over `additionalManifestEntries` so deduplication uses the complete generated shell manifest and Workbox reports actual added sizes. Required order: isolated graph RED, implementation/GREEN, real production build and unchanged cold-editor offline journey, native-disabled configuration/build check, broad gates and current-head CI. Detailed steps: [PWA delivery plan](../../docs/superpowers/plans/2026-09-09-pwa-diary-offline-delivery.md).

Build-graph refinement before runtime acceptance: the full editor closure also reaches 695296 bytes of shared initial-boot dependencies already covered by the existing controlled, visited-route flow. Adding them would expand this into shell-cache redesign. Subtract the exact emitted initial-entry static graph, fail on unresolved boot edges or an eager editor, and retain the 512 KiB cap for the cold delta (345090 bytes before Vite's final preload rewrites). Run collection at writeBundle to bind final bytes. Four new behavioral regressions were RED before this refinement; no runtime/persistence assertion or budget was relaxed.

The saved-entry reload additionally requires the existing lazy JournalEntryList and its static dependencies; a second real browser RED identified that root. With it, final emitted cold assets total 566129 bytes, including the existing dark-theme image. The preliminary 512 KiB guard blocked production output. The owner approved T023 at 16:08 UTC: the current cap is 640 KiB (655360 bytes), retaining the image and all persistence/visual checks. The earlier editor-only and 512 KiB figures above document the observed design sequence, not the current limit. Do not publish the prior partial editor-only build.

Five-platform impact of this follow-up: Web/Vite and browser/installed PWA gain only installation assets; installed OS update lifecycle remains separately unverified. Capacitor Android/iOS retain disabled-worker packaging. Desktop/Tauri retains existing environment-controlled behavior; packaged runtime remains unverified. No user-data/cache deletion or forced update is introduced. Rollback is an ordinary code/config revert, with the existing worker update lifecycle and IndexedDB records preserved.
