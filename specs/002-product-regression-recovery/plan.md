...
**Current branch delivery**: PR 1 / Wave 1 only. Wave 2 and Wave 3 designs remain in this plan, but their production files are not part of this branch. Their earlier combined draft is retained locally at `9b89931a10db29f654dd9239b266f530d55fa4a8` and must be reworked from accepted `main` after Wave 1.
...
4. Keep schema v1 non-enabling even when every tracked status says `pass`; a later separately reviewed schema may enable only after exact evidence hashes and explicit owner authority are verifiable for the same commit/artifact set.
