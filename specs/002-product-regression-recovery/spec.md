...
**Status**: In progress — Waves 1/2 and a non-enabling Wave 3 candidate are integrated locally; final verification, delivery, and human/device gates remain
...
- **FR-003**: The password-removal contract MUST distinguish `ready`, `unlock-required`, `activation-pending`, `removal-pending`, `vault-revision-mismatch`, `decrypt-entry`, `decrypt-media`, `decrypt-draft`, `decrypt-space`, `decrypt-capture`, `owner-adoption-pending`, `owner-changed`, `fresh-auth-required`, and `storage-failed` outcomes.
...
- **FR-076**: An authenticated session whose local owner adoption has not completed MUST stop before creating a removal intent or calling the remote fence, return `owner-adoption-pending`, preserve every protected artifact, and permit a fresh retry only after the normal account lifecycle confirms adoption.
