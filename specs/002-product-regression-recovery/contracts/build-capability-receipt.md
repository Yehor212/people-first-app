...
- `journalSaveCeremony` may be true only when all six admission values are `pass` for the same commit and artifact set and the kill switch is false.
...
- A validator rejects schema drift, forbidden fields, unknown platforms, source-commit mismatch, target disagreement, or an enabled capability with any non-pass admission row.
...
- Any `fail` or `unverified` admission row forces disabled.
