...
2. Create or resume the exact owner/revision-bound durable operation without overwriting an unresolved or unknown-version intent.
3. Read and prepare entries, photos, audio, drafts, spaces, and captures. Metadata-only media is downloaded into operation-scoped memory; missing/decrypt-failed bytes block as `decrypt-media`.
4. Return a typed non-ready result without changing rows, password/vault metadata, native credential, queue, or cloud delivery.
5. Build a privacy-safe inventory of exact row hashes and Storage identities, then acquire the owner/revision/operation-bound server fence. A fence miss changes no local protected row.
6. Under the existing `DATA -> JOURNAL` lock order, revalidate owner, vault revision, operation revision, and raw row snapshots.
7. In one Dexie transaction, write every prepared local plaintext object, remove local password/vault/biometric-setting/cooldown metadata, and persist `local-committed`. Abort means none of these writes commit.
8. Revalidate owner immediately before clearing the installation-wide native biometric credential. A failure becomes pending cleanup; it does not reverse or misreport the local commit.
9. Enqueue/resume journal-scoped cloud finalization. Queue failure leaves the durable intent and reports `removed-cleanup-pending`.
10. App-level startup/resume processing retries cleanup without requiring the journal route to mount.
