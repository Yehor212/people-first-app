...
**Delivery note**: Decisions 1-10 and 12 inform the journal recovery. Feature availability and the non-enabling Wave 3 capability/host candidate are implemented locally. Ceremony production admission remains blocked until exact-commit, device, artistic/craft, motion, and explicit owner gates close.
...
**Decision**: Export stable blocker classes only: `unlock-required`, `activation-pending`, `removal-pending`, `vault-revision-mismatch`, the five decrypt categories, `owner-adoption-pending`, `owner-changed`, `fresh-auth-required`, and `storage-failed`. Keep the failing record identifier, ciphertext, content, key, and raw exception out of consumer results and logs. Owner adoption remains exclusively owned by the auth/account lifecycle; journal removal observes it and fails closed rather than adopting potentially foreign local data.
