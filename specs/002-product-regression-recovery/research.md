...
**Decision**: Add an exact build capability receipt and one explicit release input across target build paths, but keep its production value false. A later isolated release commit may enable it only after technical, accessibility, performance, visual-runtime, artistic/craft, and explicit owner visual approval all bind to the same artifact hashes and commit.

**Rationale**: The assets, lazy build guard, lifecycle token, static reduced-motion variant, and runtime circuit breaker already exist. What is missing is release admission, not another animation implementation. Technical rendering cannot prove visual quality or user acceptance.
...
**Verification path**: Exact build receipt checks for Web/Android/iOS/Desktop, lifecycle/strain tests, browser screenshots/traces, independent visual critic, then explicit owner decision.

## Decision 8 — Modal Behavior Uses Existing Focus Infrastructure

**Decision**: Keep the existing focus trap, least-destructive initial focus, `role="dialog"`, `aria-modal`, Android back hook, and 44 px targets. Add status-specific live messaging and ensure Escape/Back cannot close during a mutating stage but can close after a safe blocker or partial result.
...
**Verification path**: Static migration contract tests cover default/allowed modes, explicit strict activation inventory, protected-write pause, plaintext conversion admission, grants, and restoration after finalization. PostgreSQL execution, RLS behavior, old/new-client coexistence, per-owner strict promotion, and migration rollback drills remain `UNVERIFIED` until run against an authorized non-production Supabase target.
