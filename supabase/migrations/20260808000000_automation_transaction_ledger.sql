...
-- Rollback is forward-only once any v11 client has written history. Kill new
-- writes first by enabling the automation.connected-records.v1 killswitch and
-- setting rollout to zero, then deploy a v11-aware replacement that rejects
-- commit, undo and purge. Preserve ledger/tombstone state and repair only with
-- a reviewed forward migration; never remove the tables with a down migration.
...
CREATE TABLE public.automation_mutations (
  ...
  FOREIGN KEY (user_id, transaction_id)
    REFERENCES public.automation_transactions(user_id, id) ON DELETE CASCADE,
  CHECK (
...
BEGIN
  ...
  IF EXISTS (
    ...
    GROUP BY mutation.value->>'entityType', mutation.value->>'entityId'
    HAVING count(*) > 1
  ) THEN
    RETURN public.automation_commit_rejection(
      'INVALID_REQUEST', v_transaction_id, 0, 1
  ...
  IF NOT FOUND THEN
    RETURN public.automation_commit_rejection(
      'STALE_CONSENT_EPOCH', v_transaction_id, 0, 1
    );
  END IF;
  ...
  IF NOT FOUND THEN
    RETURN public.automation_commit_rejection(
      ...
      v_preference.server_revision, 1
    );
  END IF;
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN public.automation_undo_rejection(
      'HISTORY_GENERATION_STALE', v_transaction_id, 1
    );
  END IF;
