-- Bind a push claim to the same owner that passed the local account boundary.
-- A replacement JWT must not be able to claim an installation with stale
-- consent from the previous locally admitted account.

BEGIN;

DROP FUNCTION IF EXISTS public.claim_push_install(text, text, text);

CREATE FUNCTION public.claim_push_install(
  p_token text,
  p_device_id text,
