...
const sendFcmNotifications = async (targets: PushDeviceTarget[], type: "test") => {
  let accessToken: string | null | undefined;
  try {
    accessToken = await getFcmAccessToken();
  } catch {
    console.warn("[SendPushTest] FCM credentials unavailable");
  ...
  if (dispatch.state === "provider-unavailable") {
    console.warn("[SendPushTest] FCM batch unavailable");
  }
  return dispatch;
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [, bodyErr] = await parseJsonBody<Record<string, unknown>>(req, origin);
    ...
    const delivery = await withPushDeliveryPermit(
      ...
      async () => {
        const { data: deviceTokens, error: tokenError } = await supabase
          ...
          .eq("platform", "android");
        if (tokenError) {
          console.warn("[SendPushTest] Push targets unavailable");
          return providerUnavailable(0);
        }

        const targets = (deviceTokens || []).filter(isAndroidPushTarget);
        if (targets.length === 0) return noPushTargets();
        return await sendFcmNotifications(targets, "test");
      },
    );
    ...
    if (!delivery.releaseConfirmed) {
      console.warn("[SendPushTest] Permit release unconfirmed");
    }
    ...
    console.error("[SendPushTest] Internal failure");
