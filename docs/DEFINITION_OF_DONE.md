...
| 17 | Google Play AdMob/app-ads production contract | Current scope is one Android anchored adaptive Habits banner. Generate and verify the publisher file with `google-play:app-ads` and `google-play:app-ads:check`. `google-play:admob:check` requires real owner-controlled Android app and banner IDs and rejects sample/test/mismatched IDs; `google-play:admob:ump-check`, `google-play:admob:owner-runbook:check`, owner evidence validation, `google-play:admob:external-check`, and the fail-closed readiness command `google-play:admob:external-check:pass` remain separate gates. Public listing/privacy/app-ads checks, `google-play:privacy:artifact-check`, the post-deploy public privacy smoke, exact release manifest/bundle scans, Play Ads/Advertising ID/Data safety declarations, AdMob readiness/Policy Center/CMP, and public-safe account readiness are also required. `live_ad_playback_device` requires the exact Play-installed artifact, consent, banner render, AdMob request and impression, no overlap, rotation/background/revocation, and all protected-surface checks. Missing evidence remains `UNVERIFIED`; static/build evidence never proves live serving. Web/PWA/iOS/Desktop remain ad-free in this release. | Yes for Google Play, Android ads, AdMob, Advertising ID, or production monetization claims |
