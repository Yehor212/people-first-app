...
| 16 | Microsoft Store/MSIX contract | `npm run desktop:store:package` and `npm run desktop:store:check`; Store package identity must come from Partner Center Product Identity, Store package/certification state stays `UNVERIFIED` until the generated package is accepted in Partner Center and Windows App Certification Kit or Store certification evidence exists | Yes for Partner Center, Store, MSIX, or Store-ready claims |
