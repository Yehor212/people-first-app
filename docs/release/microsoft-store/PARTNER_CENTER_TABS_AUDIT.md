...
| Store logos | `PASS` | Official logo assets are committed; Partner Center logo section proof exists in `tmp/partner-center-store-logos-visible.png`. | Keep official-logo assets; do not reintroduce old orb-led draft exports without owner approval. |
| Packages | `BLOCKED_UNTIL_PACKAGE_UPLOAD` | Partner Center showed `Packages` as `Not started`; `npm run desktop:store:package` now generates the Store `.msixupload` candidate. | Upload `tmp/microsoft-store-msix/ZenFlow_2.0.0.0_x64.msixupload`, capture Partner Center acceptance proof, then review package languages. |
