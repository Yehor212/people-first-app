...
`max_concurrent_threads_per_session = 3` is the canonical Codex concurrency
assignment. It limits concurrently open spawned specialist threads only when the
project config is loaded; effective runtime loading and enforcement remain
`UNVERIFIED`. `specialist_depth = 1` and forbidden recursive fanout are project
routing policies. No native depth assignment is generated, so runtime enforcement of
those policies also remains `UNVERIFIED`.
