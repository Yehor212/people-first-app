...
| Google Play/AdMob production | Current scope is the Android Habits banner only. Run `google-play:app-ads`, `google-play:app-ads:check`, `google-play:admob:check`, `google-play:admob:ump-check`, `google-play:admob:owner-runbook:check`, `google-play:admob:owner-next-steps`, `google-play:admob:owner-next-steps -- --require-support-ready`, `google-play:admob:owner-evidence:check`, `google-play:admob:owner-evidence:prepare`, and the dry-run `google-play:admob:owner-evidence:apply` independently. Keep `google-play:admob:external-check` informational until every external row is current; only `google-play:admob:external-check:pass` can admit a readiness claim. Also require `google-play:privacy:artifact-check`, the post-deploy public privacy smoke, public listing/privacy/app-ads checks, exact manifest/bundle scans, and Play/AdMob console gates. Production IDs must be owner-controlled and non-sample; no fallback or synthetic ad data may enter runtime or bundles. `live_ad_playback_device` requires a Play-installed production artifact, consent, banner render, AdMob request and impression, no overlap, rotation/background/revocation, and protected-surface checks. Any missing public, console, account, artifact, or runtime proof is `UNVERIFIED`, never PASS. |
