...
| Google Play/AdMob production | `npm run google-play:assets:check`, real AdMob app/ad unit IDs from owner-controlled env, `public/app-ads.txt` generated by `npm run google-play:app-ads` with the real `ZENFLOW_ADMOB_PUBLISHER_ID`, verified by `npm run google-play:app-ads:check`, Android release manifest proof, and current Play Console Ads/Advertising ID/Data safety proof. Missing publisher/app/ad-unit IDs or Play Console proof is `UNVERIFIED`, never PASS. |
