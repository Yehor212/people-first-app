...
| Google Play/AdMob production | `npm run google-play:assets:check`, `npm run google-play:admob:check`, real AdMob app/ad unit IDs from owner-controlled env, `public/app-ads.txt` generated by `npm run google-play:app-ads` with the real `ZENFLOW_ADMOB_PUBLISHER_ID`, verified by `npm run google-play:app-ads:check`, live root-domain proof with `ZENFLOW_APP_ADS_PUBLIC_URL=https://your-developer-domain.example/app-ads.txt npm run google-play:app-ads:public-check`, public Play listing proof with `npm run google-play:public-listing:check`, Android release manifest proof, and current Play Console Ads/Advertising ID/Data safety proof. Missing publisher/app/ad-unit IDs, public crawler proof, or Play Console proof is `UNVERIFIED`, never PASS. |
