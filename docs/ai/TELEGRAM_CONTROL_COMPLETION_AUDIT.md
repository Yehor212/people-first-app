...
| Local Worker/package verification | PASS | `npm run check:telegram-control`: 75 worker tests, workflow invariants, local smoke, setup verifier, KV setup dry-run, GitHub App manifest dry-run, Telegram doctor dry-run, activation runner dry-run, activation doctor, webhook dry-run, bot UI dry-run, generated-secret dry-run, account-secret dry-run, and generated-secret install dry-run ran |
...
| Telegram bot readiness doctor exists | PASS | `tools/telegram-control/src/telegram-readiness.ts`, `tools/telegram-control/scripts/telegram-doctor.ts`, and `npm --prefix tools/telegram-control run telegram:doctor` validate token shape, webhook secret-token rules, admin allowlist, webhook URL, and Mini App URL without printing secrets; `--live` checks Bot API `getMe` and `getWebhookInfo` |
...
| GitHub App manifest helper exists | PASS | `tools/telegram-control/src/github-app-manifest.ts`, `tools/telegram-control/scripts/github-app-manifest.ts`, and `npm --prefix tools/telegram-control run github-app:manifest -- --base-url https://example.invalid` generate the owner-controlled GitHub App manifest without App ID/private key/webhook secret values |
| Activation runner exists | PASS | `tools/telegram-control/src/activation-runner.ts`, `tools/telegram-control/scripts/activation-run.ts`, and `npm --prefix tools/telegram-control run activation:run` compose KV, account secrets, generated secrets, deploy, GitHub App manifest, callback URL, Telegram bot readiness, Telegram webhook/UI, live smoke, and doctor steps with dry-run default |
...
3. Generate the GitHub App manifest with `npm --prefix tools/telegram-control run github-app:manifest -- --base-url https://<worker-host>` after the Worker URL exists, then complete GitHub's owner-controlled manifest flow and install returned App credentials into Cloudflare.
4. Deploy the Worker.
5. Run `npm --prefix tools/telegram-control run set-webhook` from a local shell with Telegram env vars set, then `npm --prefix tools/telegram-control run telegram:doctor -- --live` to verify Bot API `getMe` and `getWebhookInfo` without printing the token.
6. Configure `TELEGRAM_CONTROL_CALLBACK_URL` and `TELEGRAM_CONTROL_CALLBACK_SECRET` in GitHub.
7. Optionally add GitHub `OPENAI_API_KEY` with `npm --prefix tools/telegram-control run secrets:install-account -- --github --github-openai` only after explicit key approval; without it, AI modes correctly return `UNVERIFIED`.
8. Send `/health` and `/status` from the allowlisted Telegram account and save the responses as live evidence.
9. Run `npm --prefix tools/telegram-control run set-bot-ui` after `set-webhook` so Telegram shows the command menu and Mini App button.
10. Run `npm --prefix tools/telegram-control run activation:run -- --apply --kv --cloudflare-account-secrets --cloudflare-secrets --github-snyk-secret --github-secrets --deploy --github-callback --telegram --live-smoke --external-checks` only after all env vars and CLIs are ready.
11. Run `npm --prefix tools/telegram-control run smoke:live` and save the PASS output as deployed Worker evidence.
