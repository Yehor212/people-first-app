...
The owner-approved 2026-09-10 work budget is **128 MiB (134,217,728 bytes) per bundle root**, retaining at most four explicit roots, 2,048 directories and 10,000 files per root, and 8 MiB per artifact/read. This accommodates the unchanged R7 collection in the approximately 90 MiB app bundle without re-encoding, skipping binary media, or weakening content/hash checks. It increases worst-case aggregate scan work, not the size of an individual file buffer. Every artifact remains inventoried, byte-hashed and scanned, followed by a fresh inventory comparison. Tests admit clean 80/128 MiB bundles, reject 128 MiB plus one byte and 8 MiB plus one byte per artifact, and detect a fixture canary and post-inventory mutation at the end of a 128 MiB media bundle. Actual scan time and peak memory must be recorded for release evidence; a successful boundary test alone is not production verification. No exclusions, baselines, waivers, hooks or CI contracts change with this budget revision.
