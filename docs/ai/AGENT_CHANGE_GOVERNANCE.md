...
This is an operator protocol, not application runtime code. It applies to Codex project roles, built-in workers, local subagents, and connector-backed agents.
...
Use subagents when a distinct evidence question materially improves quality, and keep the set no larger than necessary. Each subagent must return:
