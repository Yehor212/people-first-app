...
| 2026-04-19 | (this commit) | **teamlead batch — revolution ergonomics (research-backed).** Replaces the always-visible `⋯` `HabitActionsMenu` button at `end-2 top-2` with a new `HabitActionSheet` Vaul drawer opened via long-press / Enter / Space. Card surface returns to V1 pristine state: chain dots (7-day `<ul>` + weekly count `N/7`) removed, identity-verb badge removed (already rendered once in `HeroIdentityPrompt` at the top of the hero zone — de-duplicated). Reminder-time pill stays inline. V1 `CompactHabitCard` is not touched (Law 1 preserved). Research base cited in `docs/audits/habits-v2-revolution-ergonomics-proposal-2026-04-19.md` (Hoober thumb-zone n=1333 → top-right is red for 67 % right-thumb users; Streaks Apple Design Award 2016 gesture hierarchy; Atoms James Clear identity framing; UX Psychology on Snackbar+UNDO vs confirm dialogs; Apple HIG Action Sheets). Item list: Skip/Unskip · Archive/Unarchive · Edit · Open details — Delete kept exclusively on V1 swipe-reveal (2-tap confirm, 3-sec window) so there is one safe destructive path instead of two. Two new i18n keys (`navV2HabitsActions`, `navV2HabitsOpenDetails`) × 8 languages = 16 entries with native translations. 8 new `HabitActionSheet` unit tests + `HeroHabitRow.test.tsx` gains action-sheet open test + fallback-to-detail test. `a11y-smoke.test.tsx` §11 #13 repositioned to assert parent `aria-label` only (chain no longer renders). `HeroHabitRow.tsx:327` hardcoded English `aria-label` (`"{N} of 7 this week"`) naturally disappears with the chain. 97/97 habits vitest green; tsc + eslint + i18n:check (2585 × 8) clean. Phase 2 deferred (not in this commit): Snackbar + UNDO system for Archive/Skip reversal, confirm-dialog with type-to-confirm for Delete of habits with > 21 days of entries, `HeroInsightStrip` 7 missing insight i18n keys. |
