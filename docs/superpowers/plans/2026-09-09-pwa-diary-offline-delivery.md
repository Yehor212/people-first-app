...
- [x] Implement collection using `moduleIds`/`facadeModuleId`, `imports` and `viteMetadata.importedCss/importedAssets`, with a visited Set. Build Workbox-compatible cache identifiers with `createHash("md5").update(bytes).digest("hex")`; measure bytes with `Buffer.byteLength`. SHA-256 continues to bind the separately sealed release artifact. Throw for a missing emitted edge, missing root or oversized file.
...
Release-format follow-up at 17:13 UTC: CI on 9502fc46 exposed the existing staging verifier's MD5 Workbox-revision contract. The collector now follows that format; SHA-256 release seals and verifier behavior are unchanged. The new cross-boundary/tamper regression was RED before the helper change. Fresh full tests passed 10364 cases; 55 collector/release tests and 1024 release/workspace cases passed. Actual prepare, stage and all 78 staged-cache byte checks passed, as did three prepared-build browser repetitions and the separate staged-package journey. No application, image, payload-size or native behavior changed.
