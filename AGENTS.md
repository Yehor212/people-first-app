...
- `config/persistent-agent-orchestra.json` is the only role-prompt source for exactly ten project custom roles; `.codex/agents/*.toml` and `docs/ai/PERSISTENT_AGENT_ORCHESTRA.md` are generated and must not be hand-edited.
...
- When a user names plugins or skills such as `@superpowers`, `@chrome`, `@browser`, `@openai-developers`, `$skill`, or `/skills`, read and follow `docs/ai/SKILL_ROUTING_AGENT_POLICY.md`; do not run every skill inside a plugin by default; select the minimal relevant skill set, read each selected `SKILL.md` completely, state the order of use, and explain obvious skipped skills. Codex project hooks are registered in `.codex/hooks.json`; `.codex/hooks/skill-router-gate.cjs` injects a routing checklist and guards protected edits. Before guarded Codex edits, provide fresh `.skill-routing-token` evidence or a structured `.preflight-token` with `skill_routing`. Use Browser for local/public runtime checks, Chrome for existing user Chrome state, and Computer Use only for real desktop UI tasks; do not automate Codex itself.

## Spec Kit Workflow Routing

- Nontrivial ZenFlow features and system changes use the explicit full route: `$speckit-specify` -> `$speckit-clarify` -> `$speckit-plan` -> `$speckit-checklist` -> `$speckit-tasks` -> `$speckit-analyze` -> authorized `$speckit-implement` -> `$speckit-converge`. Small local fixes use a compact test-first route: name expected behavior and risk, capture the smallest RED or characterization, make the minimal patch, rerun the same evidence GREEN, run blast-radius checks, and review the final diff/status; do not manufacture feature artifacts or spend the full-flow token budget while the risk remains local. Protected/high-risk changes use the full route plus M2 governance, rollback, complete platform/data/security/privacy/accessibility/performance/operations matrices, applicable Role 8/10 evidence, staged rollout and recovery where relevant, and fresh proof or `UNVERIFIED`. Run `.specify/scripts/bash/check-zenflow-constitution-status.sh --json` before using constitution content as criteria; while proposed and unratified, it is advisory and cannot be the sole blocking or `CRITICAL` authority. Optional extensions and extension hooks remain disabled; any change requires explicit user authorization, independent security review, official CLI compatibility proof, and renewed discovery. Spec Kit artifacts are plans/evidence, never authorization for production writes, deletion, deployment, dependency installation, issue creation, connector/MCP side effects, or handling raw private logs, Sentry payloads, or user content. `$speckit-taskstoissues` and every external write remain explicit user-invoked actions with a verified target. Spec Kit complements the smallest sufficient specialist set; it does not require all ten roles by ritual. `.codex/hooks/spec-kit-safety-gate.cjs` is defense in depth for supported local events and targets; it does not prove complete shell parsing, hosted-tool coverage, atomic prevention, external-client enforcement, or universal obedience by generated skills.
...
- Agent guidance checks: `npm run check:agent-context`, `npm run check:agent-orchestra`, `npm run check:agent-orchestra:eval`, `npm run ai:context:check`, `npm run ai:context:auto-check`.
