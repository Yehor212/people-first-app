...
export function buildTraceSummaryQueries(packageName) {
  ...
  return {
    frameTimeline: `WITH app AS (SELECT upid FROM process p WHERE ${processFilter} LIMIT 1), frames AS (SELECT ts,dur,jank_type,LAG(ts) OVER (ORDER BY ts) AS previous_ts FROM actual_frame_timeline_slice WHERE upid=(SELECT upid FROM app)) SELECT COUNT(*) AS appFrames,ROUND((MAX(ts)-MIN(ts))/1e9,6) AS spanSeconds,ROUND(COUNT(*)/((MAX(ts)-MIN(ts))/1e9),6) AS presentedFps,SUM(CASE WHEN INSTR(jank_type,'App Deadline Missed')>0 THEN 1 ELSE 0 END) AS appDeadlineMissed,ROUND(100.0*SUM(CASE WHEN INSTR(jank_type,'App Deadline Missed')>0 THEN 1 ELSE 0 END)/COUNT(*),6) AS appDeadlineMissedPct,ROUND(AVG(dur)/1e6,6) AS averageFrameTimelineDurationMs,ROUND(PERCENTILE(dur/1e6,95),6) AS p95FrameTimelineDurationMs,ROUND(PERCENTILE(dur/1e6,99),6) AS p99FrameTimelineDurationMs,ROUND(MAX(dur)/1e6,6) AS maxFrameTimelineDurationMs,SUM(CASE WHEN previous_ts IS NOT NULL AND ts-previous_ts>100000000 THEN 1 ELSE 0 END) AS presentationGapsOver100Ms FROM frames;`,
