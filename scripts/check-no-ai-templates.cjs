...
const TEMPLATE_MARKERS = [
  ...
  { label: "AI-generated placeholder", pattern: /\bAI-generated (?:placeholder|template|content)\b/i },
  { label: "TODO replace real content", pattern: /TODO:\s*replace (?:with )?(?:real|actual|this)/i },
...
function walkFiles(rootDir, dir = rootDir, files = []) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      if (!SKIP_PREFIXES.some((prefix) => relative === prefix.slice(0, -1) || relative.startsWith(prefix))) {
...
function isDurableTextSurface(relativePath) {
  ...
  if (!SCAN_PREFIXES.some((prefix) => normalized === prefix || normalized.startsWith(prefix))) return false;
...
function validateNoAiTemplatesPolicy(options = {}) {
  ...
  requireIncludes(".github/workflows/drift-checks.yml", driftWorkflow, [
  ...
  requireIncludes("scripts/check-agent-context.mjs", agentContext, [
    "assertNoAiTemplatesGateContract",
    "NO_AI_TEMPLATES_AGENT_POLICY.md",
    "npm run check:no-ai-templates",
  ], failures);

  requireIncludes("scripts/__tests__/no-ai-templates-policy.test.ts", testFile, [
    "Source Evidence",
    "No AI-template output",
  ...
  } else if (packageJson.scripts["check:no-ai-templates"] !== "node scripts/check-no-ai-templates.cjs") {
    failures.push('package.json script "check:no-ai-templates" must run node scripts/check-no-ai-templates.cjs');
  }

  if (!String(packageJson?.scripts?.["ci:preflight"] || "").includes("npm run check:no-ai-templates")) {
