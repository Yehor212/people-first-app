...
const MAX_BUNDLE_AGGREGATE_BYTES = 128n * 1024n * 1024n;
...
function maskSqlRangePreservingLines(value, start, end) {
  return value.slice(0, start) + value.slice(start, end).replace(/[^\n]/g, " ") + value.slice(end);
}
...
function sqlCallsRoutine(sql, normalizedName) {
  ...
  const invocationPattern = new RegExp(`\\b(?:${routineNamePattern})\\s*\\(`, "gi");
  let match;
...
function maskInertSqlRoutineBodies(sql) {
  ...
  for (const routine of [...routines].sort((left, right) => right.bodyStart - left.bodyStart)) {
    ...
    immediateSql = maskSqlRangePreservingLines(immediateSql, routine.bodyStart, routine.bodyEnd);
  }
  return immediateSql;
}
