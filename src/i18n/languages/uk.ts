...
export const uk: Translations = {
  ...
  installOnMac: "Встановити на Mac",
  installOnMacStorageWarning:
    "Дані, збережені в Safari, не переносяться автоматично до встановленої програми на Mac, а програма може попросити увійти знову. Перед встановленням залиште дані в Safari й збережіть резервну копію ZenFlow, навіть якщо ви користуєтеся акаунтом. Онлайн-резервне копіювання може бути призупинене, недоступне або не містити останніх змін. Резервна копія ZenFlow не містить усіх налаштувань, які зберігаються лише на цьому пристрої. Відкривши встановлену програму, увійдіть знову, якщо хочете відновити дані з онлайн-резервної копії. Щоб натомість скористатися вручну збереженим файлом резервної копії, імпортуйте його до підключення акаунта. Потім перевірте потрібні дані, перш ніж щось видаляти із Safari.",
  installOnMacSafariSteps:
    "У macOS Sonoma 14 або новішій у Safari виберіть «Файл» → «Додати до Dock» або натисніть кнопку «Поширити» на панелі інструментів і виберіть «Додати до Dock». Якщо жодної з цих команд немає, продовжуйте користуватися ZenFlow у Safari.",
  // App Updates (v1.4.1)
