...
export const MindMapCanvas = forwardRef<MindMapCanvasRef, MindMapCanvasProps>(
  function MindMapCanvas(
    ...
    const handleGoalNodeTap = useCallback(
      (goalId: string) => {
    ...
    const handleGoalFlowSubmit = useCallback(
    ...
    const handleGoalUpdateIcon = useCallback(
      (goalId: string, icon: string | undefined) => {
        onGoalUpdateIcon(goalId, icon);
      },
      [onGoalUpdateIcon]
    );

    const handleGoalUpdateEmoji = useCallback(
      (goalId: string, emoji: string | undefined) => {
        onGoalUpdateEmoji(goalId, emoji);
      },
      [onGoalUpdateEmoji]
    );

    const handleGoalUpdateColor = useCallback(
      (goalId: string, color: string | undefined) => {
    ...
    return (
      <div
        ...
        className="absolute inset-0 overflow-hidden bg-background touch-none overscroll-contain"
        onWheel={handleWheel}
