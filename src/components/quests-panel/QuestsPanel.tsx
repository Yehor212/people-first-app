...
export function QuestsPanel({ onClose }: QuestsPanelProps) {
  ...
  const handleRefreshDaily = useCallback(() => {
    ...
    dailyTimerRef.current = setTimeout(() => {
      refreshDebounceRef.current.daily = false;
    }, 300);
  }, []);
  ...
  const handleRefreshWeekly = useCallback(() => {
    ...
    weeklyTimerRef.current = setTimeout(() => {
      refreshDebounceRef.current.weekly = false;
    }, 300);
  }, []);
  ...
  const handleGenerateBonus = useCallback(() => {
    ...
    if (!bonusQuest || shouldRegenerateQuest(bonusQuest)) {
      ...
      bonusTimerRef.current = setTimeout(() => {
        refreshDebounceRef.current.bonus = false;
      }, 300);
    }
  }, [bonusQuest]);
  ...
  return (
    <div
      ...
      className="fixed inset-0 lg:mx-auto lg:my-8 lg:max-w-2xl lg:rounded-2xl lg:shadow-2xl z-[60] bg-background/95 backdrop-blur-sm overflow-y-auto"
    >
