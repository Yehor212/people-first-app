...
export function HyperfocusMode({
  duration,
  onComplete,
  onExit,
}: HyperfocusModeProps) {
  ...
  const audio = useHyperfocusAudio({
    isRunning: timer.isRunning,
    isPaused: timer.isPaused,
  });
  const dnd = useHyperfocusDnd();
  const spotify = useHyperfocusSpotify({
    isRunning: timer.isRunning,
    isPaused: timer.isPaused,
  });
  ...
  const progressColor = useMemo(() => {
    if (timer.progress < 70)
      return {
        from: "hsl(var(--focus-violet))",
        to: "hsl(var(--focus-purple))",
      };
    if (timer.progress < 90)
      return {
        from: "hsl(var(--focus-pink))",
        to: "hsl(var(--focus-pink-mid))",
      };
    return {
      from: "hsl(var(--focus-orange))",
      to: "hsl(var(--focus-orange-light))",
    };
  }, [timer.progress]);
  ...
  return (
    <div className="fixed inset-0 bg-slate-100 dark:bg-black z-[110]">
      <HyperfocusBackground
        showBreathingAnimation={timer.showBreathingAnimation}
        t={tRecord}
      />
              ...
              >
                  ...
                  <div className="text-start">
                    ...
                    {dnd.dndEnabled && (
                      <p className="text-xs text-violet-600/70 dark:text-violet-300/60">
                        {t.focusModeEnabled ||
                          "Focus mode on — distractions silenced"}
                      </p>
                    )}
                ...
                >
                  <motion.div
                    ...
                    animate={{
                      marginInlineStart: dnd.dndEnabled ? "22px" : "2px",
                    }}
            ...
            <div className="w-full max-w-sm sm:max-w-md lg:max-w-lg mx-auto mt-4 bg-secondary backdrop-blur-md rounded-2xl p-4 border border-border">
              <div className="flex items-center justify-between mb-3">
                ...
                {spotify.spotifyConnected && (
                  <motion.button
                    onClick={() =>
                      spotify.setSpotifyAutoPlay(!spotify.spotifyAutoPlay)
                    }
                    className={cn(
                  ...
                  {spotify.spotifyTrack.isPlaying && (
                    <div className="flex gap-0.5 items-end h-5">
                      ...
                      <motion.div
                        className="w-1 bg-[hsl(var(--brand-spotify))] rounded-full"
                        animate={{
                          height: ["16px", "8px", "20px", "12px", "16px"],
                        }}
                        transition={{
                          duration: 1,
                          repeat: Infinity,
                      ...
                      <motion.div
                        className="w-1 bg-[hsl(var(--brand-spotify))] rounded-full"
                        animate={{
                          height: ["12px", "16px", "8px", "20px", "12px"],
                        }}
                        transition={{
                          duration: 1,
                          repeat: Infinity,
