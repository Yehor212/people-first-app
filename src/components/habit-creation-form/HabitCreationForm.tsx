      ...
      <div className={cn(
        ...
        <div>
          ...
          <input
            ...
            placeholder={t.identityClusterPlaceholder || 'e.g., The Mindful Me'}
            maxLength={40}
            className={cn(
        ...
        <div>
          <label className={cn("text-xs mb-1 block", isPrimaryCTA ? "text-slate-500 dark:text-foreground/50" : "text-muted-foreground")}>
            {t.identityVerb || 'Affirmation'}
          </label>
          <input
            type="text"
            value={identityVerb}
            onChange={(e) => setIdentityVerb(e.target.value)}
            placeholder={t.identityVerbPlaceholder || 'e.g., I am a meditator'}
            maxLength={60}
            className={cn(
              "w-full p-2 rounded-lg text-sm transition-all",
              "focus:outline-none focus:ring-2",
