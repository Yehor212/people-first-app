...
export function StorageIncidentBanner({
  ...
  return (
    <div
      ...
      className="fixed bottom-[calc(5rem+var(--safe-bottom))] start-[max(0.5rem,var(--safe-inline-start))] end-[max(0.5rem,var(--safe-inline-end))] z-50 max-h-[calc(var(--app-viewport-height)-6rem-var(--safe-bottom))] overflow-y-auto overscroll-contain motion-safe:animate-slide-up min-[420px]:start-[max(1rem,var(--safe-inline-start))] min-[420px]:end-[max(1rem,var(--safe-inline-end))]"
    >
      <div className="grid min-w-0 grid-cols-1 items-start gap-3 rounded-xl bg-[hsl(var(--zf-warning))] p-2 text-[hsl(var(--zf-night-0))] shadow-lg backdrop-blur-sm min-[420px]:p-4 sm:grid-cols-[auto_minmax(0,1fr)]">
        ...
        <div className="grid min-w-0 max-w-full grid-cols-1 items-center gap-2 sm:col-span-2 sm:grid-cols-[minmax(0,1fr)_auto]">
          {retryLabel && onRetry ? (
            <button
              type="button"
              onClick={onRetry}
          ...
          <button
            ...
            className="flex min-h-[44px] min-w-[44px] flex-shrink-0 items-center justify-center justify-self-end rounded-lg p-2 hover:bg-foreground/20 motion-safe:transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
