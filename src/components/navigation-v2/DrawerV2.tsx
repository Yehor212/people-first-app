...
import { X, ChevronRight, LoaderCircle } from "lucide-react";
...
import { DrawerV2Destinations } from "./DrawerV2Destinations";
...
export const DrawerV2 = memo(function DrawerV2({
  ...
  useEffect(() => {
    ...
    // No compositor transition exists under reduced motion. Waiting for its
    // event would retain the drawer and block navigation for the watchdog.
    if (isDrawerMotionReduced()) {
      finishExit();
      return;
    }

    closeTimerRef.current = setTimeout(finishExit, DRAWER_EXIT_FALLBACK_MS);
    const finishReducedExit = () => {
      if (isDrawerMotionReduced()) finishExit();
    };
    const motionPreference = window.matchMedia?.("(prefers-reduced-motion: reduce)");
    motionPreference?.addEventListener("change", finishReducedExit);
    const motionClassObserver = new MutationObserver(finishReducedExit);
    motionClassObserver.observe(document.body, { attributes: true, attributeFilter: ["class"] });

    return () => {
      motionPreference?.removeEventListener("change", finishReducedExit);
  ...
  const content = (
    <>
      ...
      >
        ...
        <DrawerV2Destinations
          translations={tx}
          isRTL={isRTL}
          activePage={activePage}
          navigatingPage={navigatingPage}
          beginNavigationFeedback={beginNavigationFeedback}
          clearNavigatingPage={clearNavigatingPage}
