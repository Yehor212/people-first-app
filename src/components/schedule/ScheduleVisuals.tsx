...
export function EventCard3D({
  ...
  const isEffectivelyCurrent = isCurrent && !isCompleted;
  const accessibleName = isCompleted
    ? `${event.title} — ${t.completed || "Completed"}`
    : event.title;
  ...
  return (
    <motion.button
      ...
      data-current={String(isEffectivelyCurrent)}
      className={cn(
        ...
        isCompleted && "border-solid opacity-80 saturate-50",
      )}
      style={{
        left: `${left}%`,
    ...
    >
      ...
      {/* Source badges */}
      {isHabitEvent && (
        <span className="absolute -top-1 -start-1 bg-white/30 dark:bg-white/30 rounded-full w-4 h-4 flex items-center justify-center backdrop-blur-sm">
      ...
      {/* Emoji with glow */}
      <motion.span
        animate={shouldAnimate && !isCompleted ? { scale: [1, 1.1, 1] } : undefined}
        transition={shouldAnimate && !isCompleted ? { duration: 2, repeat: Infinity } : undefined}
      ...
      {isCompleted ? (
        <CheckCircle2
          ...
          aria-hidden="true"
          data-testid="schedule-event-completed-icon"
        />
      ) : null}
    </motion.button>
  );
