...
describe("Planning legacy surface reflow", () => {
  ...
  it("stacks the schedule header before its clock, title, and actions can widen the card", () => {
    ...
    expect(source).toContain(
      'className="relative w-full min-w-0 max-w-full overflow-hidden rounded-3xl motion-safe:animate-fade-in"',
    );
    expect(source).toContain('className="relative z-10 min-w-0 p-4"');
    expect(source).toContain(
  ...
  it("stacks the focus summary before translated copy can collide", () => {
    ...
    expect(source).toContain(
      "min-w-0 max-w-full flex flex-wrap items-center gap-2 whitespace-normal",
    );
