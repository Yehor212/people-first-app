...
const ProgressStoriesViewer = lazyWithRetry(
  () =>
    import("@/components/ProgressStoriesViewer").then((m) => ({
      default: m.ProgressStoriesViewer,
    })),
...
export const StatsPage = memo(function StatsPage({
  ...
  // --- Hooks ---
  const { completedFocusSessions, filteredMoods, stats, premiumStats, moodInsights } =
  ...
  const ringCurrentValue = useMemo(() => {
    ...
    const data =
      selectedRing === "mood"
        ? pageData.ringWeeklyData.mood
        : selectedRing === "habits"
          ? pageData.ringWeeklyData.habits
          : selectedRing === "focus"
            ? pageData.ringWeeklyData.focus
            : [];
  ...
  const ringPreviousAverage = useMemo(() => {
    if (!selectedRing) return 0;
    return selectedRing === "mood"
      ? pageData.ringWeeklyData.prevMoodAvg
      : selectedRing === "habits"
        ? pageData.ringWeeklyData.prevHabitsAvg
        : selectedRing === "focus"
          ? pageData.ringWeeklyData.prevFocusAvg
          : 0;
  }, [selectedRing, pageData.ringWeeklyData]);
    ...
    >
      ...
      {moods.length === 0 && habits.length === 0 && focusSessions.length === 0 && (
        <EmptyState
          ...
          hint={
            t.moodNoData ||
            "Log your mood, complete habits, or start a focus session to see data here."
          }
        />
      )}
      ...
      <UnifiedShareModal
        ...
        data={{
          ...
          stats: [
            ...
            {
              label: t.habitsCompleted || "Habits",
              value: habits.filter((h) => h.entries?.[todayKey]?.value === 2).length,
            },
