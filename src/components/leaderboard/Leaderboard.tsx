          ...
          {data.isOptedIn && currentRank && (
            ...
            >
              ...
              <span className="text-foreground/70">
                {t.yourRank || 'Your rank'}: <strong className="text-violet-700 dark:text-violet-300">{formatRank(currentRank)}</strong>
                {data.userRanks && <span className="text-foreground/40"> / {data.userRanks.totalParticipants}</span>}
              </span>
            </motion.div>
          )}
        </div>
