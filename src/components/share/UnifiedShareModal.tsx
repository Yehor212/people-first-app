...
export function UnifiedShareModal(props: UnifiedShareModalProps) {
  ...
  const generateFn = useCallback(async (): Promise<Blob> => {
    if (badgeData)
      return generateAchievementCard(badgeData, language, username);
    if (streakValue !== null)
    ...
    if (trophyData)
      return generateTrophyCard(trophyData, cardTranslations, theme, language);
  ...
  const {
    ...
    generateFn,
    errorMessage:
      t.shareGenerateError || "Failed to generate image. Try again.",
  });
