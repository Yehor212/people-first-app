...
/** Initial number of tags shown before "Show More" */
 ...
 * Shows first 8 tags with "Show More" to expand.
 *
...
export function EmotionTagGrid({ valence, selected, onToggle }: EmotionTagGridProps) {
  ...
  const visibleTags = expanded ? filteredTags : filteredTags.slice(0, INITIAL_VISIBLE);
  const hasMore = filteredTags.length > INITIAL_VISIBLE;
  ...
  return (
    <div>
      ...
      {/* Show More — Apple style: expand only, no collapse */}
      {hasMore && !expanded && (
