...
// Retained for a future Planning workspace; the live route mounts Focus only.
export const PlanningWorkspace = memo(function PlanningWorkspace({
  onCompleteFocusSession,
}: PlanningPageProps) {
  ...
  const publishDurableScheduleEvents = useUserDataStore((s) => s._publishDurableScheduleEvents);
  ...
  const handleDeleteScheduleEvent = useCallback(
    (id: string) => {
      ...
      persistManualScheduleEvents((current) => current.filter((candidate) => candidate.id !== id));
    },
  ...
  return (
    ...
    >
      <div className="relative z-10 mx-auto flex w-full max-w-6xl flex-col gap-5 md:gap-6">
        ...
        <PlanningDayPulse pulse={planningFeatureModel.dayPulse} labels={tx} language={language} />
