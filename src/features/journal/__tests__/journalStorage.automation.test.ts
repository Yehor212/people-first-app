...
describe("journal source intent atomic persistence", () => {
  ...
  it("commits the journal, ordinary sync outbox, revision and prose-free source intent together", async () => {
  ...
  it("commits an edited explicit mood and its new source revision in the update transaction", async () => {
    const entry = await saveEntry({
      date: "2026-08-08",
      title: "Before",
      content: "Private body",
