...
const mocks = vi.hoisted(() => ({
  ...
  encryptJournalContent: vi.fn((content: string, key: string) => Promise.resolve(`enc:${key}:${content}`)),
  decryptJournalContentIfNeeded: vi.fn((content: string, key: string) =>
    Promise.resolve(content.startsWith(`enc:${key}:`) ? content.slice(`enc:${key}:`.length) : content),
  ),
...
describe("journalStorage content encryption", () => {
  ...
  it("stores and syncs encrypted content while returning plaintext to the UI", async () => {
    setJournalContentVaultKey(sessionKey);
    ...
    expect(mocks.addEntry).toHaveBeenCalledWith(expect.objectContaining({
      id: "entry-id",
      content: `enc:${sessionKey}:A quiet journal line`,
    }));
    expect(mocks.syncJournalEntry).toHaveBeenCalledWith(expect.objectContaining({
      id: "entry-id",
      content: `enc:${sessionKey}:A quiet journal line`,
    }));
  });
  ...
  it("encrypts content updates before storage and sync", async () => {
    setJournalContentVaultKey(sessionKey);
    mocks.getEntry.mockResolvedValue(baseEntry({
    ...
    expect(mocks.updateEntry).toHaveBeenCalledWith("entry-id", expect.objectContaining({
      title: "Updated",
      content: `enc:${sessionKey}:Updated line`,
    }));
    expect(mocks.syncJournalEntry).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it("migrates existing plaintext entries into encrypted stored rows", async () => {
    ...
    expect(mocks.updateEntry).toHaveBeenCalledWith("plain-1", expect.objectContaining({
  ...
  it("decrypts encrypted stored rows before removing password protection", async () => {
    ...
    expect(migrated).toBe(1);
    expect(mocks.updateEntry).toHaveBeenCalledWith("enc-1", expect.objectContaining({
