...
vi.mock("@/storage/accountBoundaryRuntime", async (importOriginal) => {
  ...
  return {
    ...
    subscribeOriginAccountBoundaryGeneration: vi.fn((listener: (generation: string) => void) => {
...
function expectJournalErrorNotLogged(error: Error): void {
  expect(loggerMock.warn.mock.calls.flat()).not.toContain(error);
  expect(
    loggerMock.warn.mock.calls.flat().some((value) => String(value).includes(error.message))
  ).toBe(false);
}
...
describe("useJournal", () => {
  ...
  it("does not log raw conflict-recovery errors that may contain diary identifiers or ciphertext", async () => {
    const original = makeEntry({ id: "entry-private-id", updatedAt: 100 });
    const sensitiveRefreshError = new Error(
      "raw-id=entry-private-id ciphertext=entry-enc:vault:private"
    );
    vi.mocked(storage.getEntriesPage).mockResolvedValueOnce({
      entries: [original],
      totalCount: 1,
    ...
    vi.mocked(storage.getEntryById).mockRejectedValueOnce(sensitiveRefreshError);

    const { result } = renderHook(() => useJournal());
    await waitFor(() => expect(result.current.loading).toBe(false));

    await expect(
      act(async () => result.current.updateEntry(original.id, { content: "Local edit" }))
    ).rejects.toMatchObject({ code: "JOURNAL_ENTRY_CONFLICT" });

    expectJournalErrorNotLogged(sensitiveRefreshError);
  });
  ...
  it("keeps journal state empty until another tab finishes the account-boundary purge", async () => {
    ...
    expect(result.current.allEntries).toEqual([]);
    await waitFor(() =>
      expect(accountBoundaryRuntimeMock.waitForSettlement).toHaveBeenCalledOnce()
    );
  ...
  it("keeps readable entries visible when the opened entry becomes unreadable", async () => {
    const opened = makeEntry({ id: "corrupt-open-entry" });
    const readable = makeEntry({ id: "readable-entry", title: "Still available" });
    vi.mocked(storage.getEntriesPage)
      .mockResolvedValueOnce({
        entries: [opened],
        totalCount: 1,
    ...
    act(() => result.current.openEntry(opened.id));
    vi.mocked(storage.getEntryById).mockRejectedValueOnce(new Error("ciphertext unavailable"));

    await act(async () => {
      await runMountedDataRefresh();
    });

    await waitFor(() => expect(result.current.allEntries).toEqual([readable]));
  ...
  it("refreshes the mounted list while preserving the opened editor version for conflict recovery", async () => {
    const opened = makeEntry({
      id: "remote-update-editor",
      content: "Opened locally",
      updatedAt: 100,
    });
  ...
  it("keeps a deleted entry editor recoverable while removing the remote entry from the list", async () => {
    const opened = makeEntry({
      id: "remote-delete-editor",
      content: "Local draft base",
      updatedAt: 100,
    });
    vi.mocked(storage.getEntriesPage)
      .mockResolvedValueOnce({
  ...
  it("saves an edit against the version the user opened", async () => {
    ...
    expect(storage.updateEntry).toHaveBeenCalledWith(openedEntry.id, { content: "Edited" }, 100);
  ...
  it("keeps a recoverable load error instead of impersonating an empty diary", async () => {
    const sensitiveLoadError = new Error(
      "raw-id=entry-private-id ciphertext=entry-enc:vault:private"
    );
    vi.mocked(storage.getEntriesPage).mockRejectedValueOnce(sensitiveLoadError);
    ...
    expect(result.current.totalCount).toBe(0);
    expectJournalErrorNotLogged(sensitiveLoadError);
  ...
  it("keeps the loaded page visible when older history fails and lets refresh retry", async () => {
    const firstPageEntry = makeEntry({ id: "entry-first", createdAt: 2000 });
    const sensitiveHistoryError = new Error(
      "raw-id=entry-older-private ciphertext=entry-enc:vault:private"
    );
    vi.mocked(storage.getEntriesPage)
      .mockResolvedValueOnce({
        entries: [firstPageEntry],
      ...
      .mockRejectedValueOnce(sensitiveHistoryError)
      .mockResolvedValueOnce({
        entries: [firstPageEntry],
        totalCount: 2,
        ...readablePageMeta,
    ...
    expectJournalErrorNotLogged(sensitiveHistoryError);
  ...
  it("clears a selected-date load error when the user leaves the failed date", async () => {
    const sensitiveDateError = new Error(
      "raw-id=entry-date-private ciphertext=entry-enc:vault:private"
    );
    vi.mocked(storage.getEntriesByDate).mockRejectedValueOnce(sensitiveDateError);
    ...
    expectJournalErrorNotLogged(sensitiveDateError);
  });
  ...
  it("surfaces the unavailable count from a selected-date read", async () => {
    ...
    vi.mocked(storage.getEntriesByDate).mockResolvedValueOnce(datePage([readable], 1));
    ...
    expect(result.current.totalCount).toBe(2);
    expect(result.current.entryPageState).toBe("degraded");
    expect(result.current.entries.map((entry) => entry.id)).toEqual(["selected-date-readable"]);
  });
  ...
  it("keeps pending soft-deleted entries hidden from background and date backfills", async () => {
    ...
    await waitFor(() =>
      expect(result.current.allEntries.map((entry) => entry.id)).toEqual([pendingEntry.id])
    );
    expect(result.current.totalCount).toBe(1);
  });
  ...
  it("does not resurrect a committed delete from a stale background backfill", async () => {
    ...
    let resolveBackgroundPage: (
      page: Awaited<ReturnType<typeof storage.getEntriesPage>>
    ) => void = () => undefined;
    vi.mocked(storage.getEntriesPage)
      .mockResolvedValueOnce({
  ...
  it("clears the hidden delete marker when committed storage delete fails", async () => {
    ...
    await expect(result.current.commitDeleteEntry(retainedEntry.id)).rejects.toThrow(
      "delete failed"
    );
