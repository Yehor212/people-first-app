...
export function useJournal() {
  ...
  const setSelectedDate = useCallback((date: string | null) => {
    ...
    setSelectedDateUnavailableCount(date ? (dateUnavailableCountsRef.current.get(date) ?? 0) : 0);
        ...
        () => {
          ...
          void storage
            ...
            .then((page) => {
              ...
              startTransition(() => {
                setEntries((prev) =>
                  mergeJournalEntries(prev, page.entries, softDeletedEntryIdsRef.current)
                );
                setTotalCount((prev) =>
                  Math.max(
                    prev,
            ...
            .catch(() => {
              if (
                ...
                return;
              logger.warn("[Journal] Older diary entries are temporarily unavailable");
  ...
  const refresh = useCallback(
    async (signal?: AbortSignal) => {
      ...
      try {
        const [page, pendingDeletes, activeEntryLookup] = await Promise.all([
          storage.getEntriesPage({ limit: JOURNAL_INITIAL_ENTRY_LIMIT }),
          storage.getPendingJournalEntryDeletes(),
          activeEntryIdAtStart
            ? storage
                .getEntryById(activeEntryIdAtStart)
                .then((entry) => ({ status: "ready" as const, entry }))
                .catch(() => ({ status: "unavailable" as const, entry: undefined }))
            : Promise.resolve({ status: "ready" as const, entry: undefined }),
        ]);
        const refreshedActiveEntry = activeEntryLookup.entry;
        const activeEntryUnavailable = activeEntryLookup.status === "unavailable";
        if (activeEntryUnavailable) {
          logger.warn("[Journal] Active diary entry is unavailable during refresh");
        }
        ...
        const activeEntryStillVisible =
          refreshedActiveEntry && !softDeletedEntryIdsRef.current.has(refreshedActiveEntry.id)
            ...
            : undefined;
        startTransition(() => {
          const refreshedEntries = mergeJournalEntries(
            [],
            page.entries,
          ...
          setUnavailableCount(
            activeEntryUnavailable ? Math.max(1, page.unavailableCount) : page.unavailableCount
          );
          setInitialLoadError(false);
          setLoading(false);
        ...
        if (signal?.aborted) return;
        logger.warn("[Journal] The first diary page is temporarily unavailable");
        if (loadGenerationRef.current !== generation) return;
        startTransition(() => {
  ...
  useEffect(() => {
    ...
    const unsubscribeGeneration = subscribeOriginAccountBoundaryGeneration(() => {
      ...
      void waitForAccountBoundaryDataSettlement()
        .then(() => refresh())
        .catch(() => {
          logger.warn("[Journal] Account-boundary diary refresh remains blocked");
        });
    });
    return () => {
  ...
  useEffect(() => {
    ...
    void storage
      ...
      .catch(() => {
        ...
        logger.warn("[Journal] The selected diary date is temporarily unavailable");
  ...
  const updateEntry = useCallback(
    async (
      ...
      try {
        ...
        setEntries((prev) => prev.map((e) => (e.id === id ? { ...e, ...changes, updatedAt } : e)));
        ...
        if (storage.isJournalEntryConflictError(error)) {
          try {
            ...
            if (latest) {
              setEntries((prev) =>
                mergeJournalEntries(prev, [latest], softDeletedEntryIdsRef.current)
              );
            ...
            logger.warn("[Journal] Diary conflict recovery is temporarily unavailable");
          }
        }
        throw error;
      }
  ...
  const commitDeleteEntry = useCallback(
    async (id: string) => {
      try {
        ...
        if (status === "not-claimed") {
          ...
          if (retainedEntry) {
            setEntries((prev) =>
              mergeJournalEntries(prev, [retainedEntry], softDeletedEntryIdsRef.current)
            );
          }
  ...
  const addPhoto = useCallback(
    async (file: File, entryId: string, signal?: AbortSignal): Promise<JournalPhoto> => {
  ...
  const displayedUnavailableCount = selectedDate ? selectedDateUnavailableCount : unavailableCount;
