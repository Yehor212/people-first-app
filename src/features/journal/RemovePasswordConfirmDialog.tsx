...
interface RemovePasswordConfirmDialogProps {
  ...
  onRecoveryAction?: (
    result: Extract<JournalPasswordRemovalResult, { status: "blocked" }>,
  ) => Promise<void> | void;
...
export function RemovePasswordConfirmDialog({
  ...
  const [blockedResult, setBlockedResult] = useState<
    Extract<JournalPasswordRemovalResult, { status: "blocked" }> | null
  >(null);
  ...
  const handleRecoveryAction = async () => {
    if (!blockedResult || !onRecoveryAction || isSubmitting) return;
    setIsSubmitting(true);
    try {
      await onRecoveryAction(blockedResult);
    } catch {
      setError(
  ...
  const hasDirectReauthentication =
    blockedResult?.recoveryAction === "reauthenticate" && Boolean(onRecoveryAction);
      ...
      >
        ...
        <div className="flex min-w-0 flex-col gap-3 sm:flex-row">
          ...
          {!completionStatus ? (
            <button
              type="button"
              onClick={hasDirectReauthentication ? handleRecoveryAction : handleConfirm}
              disabled={isSubmitting}
              className={`h-auto min-h-[48px] w-full min-w-0 flex-1 whitespace-normal break-words rounded-xl px-3 py-2.5 text-sm font-medium [hyphens:manual] [overflow-wrap:break-word] disabled:cursor-not-allowed disabled:opacity-70 ${
                hasDirectReauthentication
            ...
            >
              {isSubmitting
                ? hasDirectReauthentication
                  ? ts.journalLockRemoveVerifyingAccount || "Starting verification..."
