...
interface AutomationRpcClient {
  rpc(name: AutomationRpcName, args?: Record<string, unknown>): PromiseLike<AutomationRpcResult>;
}
...
async function requireOwner(expectedOwnerUserId?: string): Promise<string> {
  const ownerUserId = await validateSyncOwner(expectedOwnerUserId, "Automation cloud operation");
  if (!ownerUserId) {
...
async function invoke(name: AutomationRpcName, args?: Record<string, unknown>): Promise<unknown> {
    ...
    for (;;) {
      ...
      if (
        ...
        (page.snapshotToken.historyGeneration !== snapshotToken.historyGeneration ||
          ...
          page.snapshotToken.recordRevisionVersion !== snapshotToken.recordRevisionVersion)
      ) {
        throw new AutomationCloudError("AUTOMATION_CLOUD_INVALID_RESPONSE");
      }
      if (!firstPage && page.allHistoryPurgedAt !== allHistoryPurgedAt) {
        throw new AutomationCloudError("AUTOMATION_CLOUD_INVALID_RESPONSE");
      }
