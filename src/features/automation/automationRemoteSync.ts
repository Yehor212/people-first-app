...
import {
  ...
  automationLocalRefreshReceiptSchema,
  ...
  type AutomationLocalRefreshReceipt,
...
export async function markAutomationDataRefreshPendingInCurrentTransaction(
  expectedOwnerUserId: string
): Promise<AutomationLocalRefreshReceipt> {
  requireReadWriteTransaction();
  const currentRow = await db.settings.get(AUTOMATION_LOCAL_REFRESH_SETTING_KEY);
  const parsed = automationLocalRefreshReceiptSchema.safeParse(currentRow?.value);
  if (currentRow && !parsed.success) {
    throw new AutomationRemoteSyncError("AUTOMATION_REMOTE_EVENT_INVALID");
  }
  const current =
    parsed.success && parsed.data.ownerUserId === expectedOwnerUserId ? parsed.data : null;
  if (current?.revision === Number.MAX_SAFE_INTEGER) {
    throw new AutomationRemoteSyncError("AUTOMATION_REMOTE_EVENT_CAPACITY");
  }
  const receipt = automationLocalRefreshReceiptSchema.parse({
    schemaVersion: 1,
    ownerUserId: expectedOwnerUserId,
    revision: (current?.revision ?? 0) + 1,
    deliveredRevision: current?.deliveredRevision ?? 0,
  });
...
export async function readPendingAutomationDataRefresh(
  expectedOwnerUserId: string
): Promise<AutomationLocalRefreshReceipt | null> {
  const currentRow = await db.settings.get(AUTOMATION_LOCAL_REFRESH_SETTING_KEY);
  ...
  if (
    parsed.data.ownerUserId !== expectedOwnerUserId ||
    parsed.data.deliveredRevision >= parsed.data.revision
  ) {
    return null;
  }
  return parsed.data;
...
export async function acknowledgeAutomationDataRefreshInCurrentTransaction(
  expectedOwnerUserId: string,
  ...
  if (current.ownerUserId !== expectedOwnerUserId) return false;
  if (!Number.isSafeInteger(deliveredRevision) || deliveredRevision < 1) {
    throw new AutomationRemoteSyncError("AUTOMATION_REMOTE_EVENT_INVALID");
  }
  if (current.deliveredRevision >= deliveredRevision) return true;
  await db.settings.put({
    key: AUTOMATION_LOCAL_REFRESH_SETTING_KEY,
    value: automationLocalRefreshReceiptSchema.parse({
      ...current,
      deliveredRevision: Math.min(current.revision, deliveredRevision),
    }),
  });
  return true;
}
...
async function alignBeforeRevisionFromAcceptedEnvelope(
  mutation: AutomationMutation,
  ownerUserId: string,
  updatedAt: number
): Promise<boolean> {
  const projection = await readLocalProjection(mutation);
  const [projectionHash, canonicalBeforeHash] = await Dexie.waitFor(
    Promise.all([hashAutomationValue(projection), hashAutomationValue(mutation.before)])
  );
  if (projectionHash !== mutation.beforeHash || canonicalBeforeHash !== mutation.beforeHash) {
    return false;
  }

  // Ordinary domain deltas intentionally do not expose revision tokens. A
  // server-accepted transaction has already fenced its exact before token and
  // hash; after authenticated decryption it may bridge that missing metadata,
  // but never a different local projection.
  if (mutation.before === null) return projection === null;
  if (projection === null || mutation.beforeRevisionToken === null) return false;

  const revisionId = recordRevisionId(mutation.entityType, mutation.entityId);
  const existing = parseStoredRevision(await db.automationTransactions.get(revisionId));
...
async function rememberAutomationDelete(mutation: AutomationMutation): Promise<void> {
    ...
    const currentMatches =
      expectedCurrent === "before"
        ? (await mutationMatchesBefore(mutation)) ||
          (await alignBeforeRevisionFromAcceptedEnvelope(mutation, ownerUserId, updatedAt))
...
type AutomationRemoteTransactionEvent = Exclude<AutomationRemoteEvent, { deliveryKind: "purge" }>;
...
async function applyRemoteTransaction(
  ...
  assertExistingTransactionCanonical(existingValue, transaction, ownerUserId);
...
type AutomationRemotePurgeEvent = Extract<AutomationRemoteEvent, { deliveryKind: "purge" }>;
...
async function applyRemotePurge(
  ...
  const selectedTransactions = all
    ? transactions.filter((transaction) => transaction.consentEpoch !== preservedConsentEpoch)
    : transactions.filter((transaction) => requested.has(transaction.id));
  const pendingTransactions = selectedTransactions.filter(
    (transaction) => transaction.status === "commit_pending" && !requested.has(transaction.id)
  );
  ...
  if (pendingTransactions.length > 0) {
    ownerRows = await db.automationTransactions.where("ownerUserId").equals(ownerUserId).toArray();
  }
  const selectedTransactionIds = new Set(selectedTransactions.map((transaction) => transaction.id));
  const ownershipIds = new Set([...result.purgedTransactionIds, ...selectedTransactionIds]);
  ...
  const removableLocalIds = ownerRows
    .filter(
      (row) =>
        ...
        (all && row.kind === "source_pending" && row.consentEpoch !== preservedConsentEpoch) ||
  ...
  const purgedTransactionIds = all
    ? undefined
    : [...new Set([...(marker.purgedTransactionIds ?? []), ...result.purgedTransactionIds])].sort();
...
export async function reconcileAutomationRemoteEventsInCurrentTransaction(
  ...
  const pending = (
    await db.automationRemoteEvents.where("ownerUserId").equals(expectedOwnerUserId).toArray()
  ).sort(
    (left, right) =>
  ...
  const snapshotEvents = pending.filter(
    (event): event is AutomationRemoteTransactionEvent => event.deliveryKind === "snapshot"
  );
  ...
  const deferred = await db.automationRemoteEvents
    ...
    .count();
  if (applied > 0) {
    await markAutomationDataRefreshPendingInCurrentTransaction(expectedOwnerUserId);
  }
  return {
    applied,
    deferred,
