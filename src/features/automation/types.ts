...
export const AUTOMATION_SOURCE_INTENT_LIMIT = 512;
export const AUTOMATION_SOURCE_RESCAN_SETTING_KEY = "automation-source-rescan-required";
...
const safeTimestampSchema = z.number().int().min(0).max(Number.MAX_SAFE_INTEGER);
...
export const automationSourceIntentSchema = z
  .object({
    ...
    candidateRuleIds: z.array(automationRuleIdSchema).min(1).max(AUTOMATION_RULE_IDS.length),
...
export const automationSourceRescanMarkerSchema = z
  .object({
    schemaVersion: z.literal(1),
    ownerUserId: z.string().uuid(),
...
export type AutomationSourceRescanMarker = z.infer<typeof automationSourceRescanMarkerSchema>;

export const AUTOMATION_LOCAL_REFRESH_SETTING_KEY = "automation-local-data-refresh-receipt";

export const automationLocalRefreshReceiptSchema = z
  .object({
    schemaVersion: z.literal(1),
    ownerUserId: z.string().uuid(),
    revision: z.number().int().min(1).max(Number.MAX_SAFE_INTEGER),
    deliveredRevision: z.number().int().min(0).max(Number.MAX_SAFE_INTEGER),
  })
  .strict()
  .superRefine((receipt, ctx) => {
    if (receipt.deliveredRevision > receipt.revision) {
      ctx.addIssue({
        code: "custom",
        path: ["deliveredRevision"],
        message: "Delivered refresh revision cannot exceed the committed revision",
      });
    }
  });

export type AutomationLocalRefreshReceipt = z.infer<typeof automationLocalRefreshReceiptSchema>;
...
export type AutomationCommitQueueIntent = z.infer<typeof automationCommitQueueIntentSchema>;
...
export type AutomationUndoQueueIntent = z.infer<typeof automationUndoQueueIntentSchema>;
...
export type AutomationHistorySnapshotToken = z.infer<typeof automationHistorySnapshotTokenSchema>;
...
export type AutomationHistorySnapshotCursor = z.infer<typeof automationHistorySnapshotCursorSchema>;
...
export type AutomationHistoryPurgeRequest = z.infer<typeof automationHistoryPurgeRequestSchema>;
...
export type AutomationHistoryPurgeResult = z.infer<typeof automationHistoryPurgeResultSchema>;
