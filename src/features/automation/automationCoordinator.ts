...
import type { AutomationServiceGateCode, AutomationServiceGateResult } from "./automationGate";
import {
...
function deniedGateResult(gate: AutomationServiceGateResult): AutomationCoordinatorResult | null {
  if (gate.allowed) return null;
  return {
    status: "deferred",
    code: gate.code === "SERVICE_ENABLED" ? "SERVICE_REFRESH_UNAVAILABLE" : gate.code,
  };
}
...
async function deleteExactIntent(
  ...
  requireBoundary(accountBoundaryGeneration, accountSessionGeneration);
...
export async function processAutomationSourceIntent(
  ...
  try {
    ...
    logger.warn("[Automation] Durable commit queue wake deferred");
  }
