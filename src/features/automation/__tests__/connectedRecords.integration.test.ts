...
vi.mock("@/storage/sync/syncOwner", () => ({
  validateSyncOwner: vi.fn(async (expectedOwnerUserId?: string) =>
    !expectedOwnerUserId || expectedOwnerUserId === runtime.ownerUserId ? runtime.ownerUserId : null
  ),
}));
...
vi.mock("@/storage/accountBoundaryRuntime", async (importOriginal) => {
  ...
  return {
    ...
    isOriginAccountBoundaryGenerationCurrent: vi.fn(
      (generation: string) => runtime.boundaryCurrent && generation === runtime.boundaryGeneration
    ),
  };
});
...
async function commitQueueRows() {
  return db.offlineQueue.where("type").equals("COMMIT_AUTOMATION_TRANSACTION").toArray();
}
...
describe("connected records local integration", () => {
  ...
  it("commits mood-to-journal once and preserves the durable outbox across restart", async () => {
    ...
    await expect(db.offlineQueue.where("entityId").equals(source.id).count()).resolves.toBe(1);
    expect(wake).toHaveBeenCalledTimes(2);

    db.close();
    await db.open();
    await expect(
      processAutomationSourceIntent(persisted.intentId!, coordinatorDependencies(), OWNER_ID)
  ...
  it("compensates an E1 optimistic commit rejected after E2 re-consent", async () => {
    ...
    await expect(
      processQueuedAutomationCommit(queueRows[0].payload as AutomationCommitQueueIntent, OWNER_ID)
    ).resolves.toEqual({ status: "obsolete", reason: "server-rejected" });

    await expect(db.moods.get(source.id)).resolves.toEqual(source);
    expect(await db.journalEntries.count()).toBe(0);
    await expect(db.automationTransactions.get(localResult.transactionId)).resolves.toMatchObject({
      status: "revoked",
      consentEpoch: CONSENT_EPOCH_1,
    });
