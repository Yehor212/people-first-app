...
describe("atomic local automation repository", () => {
  ...
  it("returns only a hash-coherent owner-bound target snapshot", async () => {
    ...
    await expect(
      readAutomationTargetSnapshot({ entityType: "journal", entityId: "journal-1" }, OWNER_ID)
    ).resolves.toEqual({
      entityType: "journal",
      entityId: "journal-1",
  ...
  it("rejects an invalid mood projection before any personal record or outbox is written", async () => {
    ...
    await expect(commitLocalAutomationTransaction(revision, commitOptions())).rejects.toMatchObject(
      { code: "AUTOMATION_COMMIT_ROW_INVALID" }
    );
  ...
  it("rejects a tombstoned deterministic transaction before any local target mutation", async () => {
    ...
    await expect(commitLocalAutomationTransaction(revision, commitOptions())).rejects.toMatchObject(
      { code: "AUTOMATION_COMMIT_TRANSACTION_PURGED" }
    );
  ...
  it("commits the primary mood and prose-free source intent in one owner-bound transaction", async () => {
    ...
    await expect(persistPrimaryRecordWithAutomationIntent(mood, intent, OWNER_ID)).resolves.toEqual(
      { intentPersisted: true, primaryInserted: true, syncOutboxPersisted: true }
    );
    ...
    await expect(db.offlineQueue.where("entityId").equals(mood.id).count()).resolves.toBe(1);
  });
  ...
  it("keeps the primary record but drops an E1 intent after consent is revoked", async () => {
    ...
    await expect(persistPrimaryRecordWithAutomationIntent(mood, intent, OWNER_ID)).resolves.toEqual(
      { intentPersisted: false, primaryInserted: true, syncOutboxPersisted: true }
    );
  ...
  it("wakes an already-mounted durable queue after an undo outbox row commits", async () => {
    const revision = await journalRevision();
    await commitLocalAutomationTransaction(revision, commitOptions());
    const pending = await db.automationTransactions.get(TRANSACTION_ID);
    if (!pending || pending.kind !== "transaction") {
      throw new Error("Expected the committed automation transaction fixture");
    }
    await db.automationTransactions.put({
      ...pending,
      status: "committed",
    ...
    await expect(db.offlineQueue.get(`automation-undo:${TRANSACTION_ID}`)).resolves.toMatchObject({
  ...
  it("rolls back an undo request when the account session changes inside its transaction", async () => {
    const revision = await journalRevision();
    await commitLocalAutomationTransaction(revision, commitOptions());
    const pending = await db.automationTransactions.get(TRANSACTION_ID);
    if (!pending || pending.kind !== "transaction") {
    ...
    await db.automationTransactions.put(committed);

    const originalPut = db.automationTransactions.put.bind(db.automationTransactions);
    const put = vi.spyOn(db.automationTransactions, "put").mockImplementationOnce((value, key) => {
    ...
    await expect(db.automationTransactions.get(TRANSACTION_ID)).resolves.toEqual(committed);
    await expect(db.offlineQueue.get(`automation-undo:${TRANSACTION_ID}`)).resolves.toBeUndefined();
    put.mockRestore();
  });

  it("rolls back a local automation commit after an ABA session transition", async () => {
    const revision = await journalRevision();
    const originalAdd = db.automationTransactions.add.bind(db.automationTransactions);
    vi.spyOn(db.automationTransactions, "add").mockImplementationOnce((value, key) => {
    ...
    await expect(commitLocalAutomationTransaction(revision, commitOptions())).rejects.toThrow(
