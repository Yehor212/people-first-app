  ...
  return db.transaction(
    ...
    async () => {
      ...
      for (const transaction of selectedTransactions) {
        ...
        if (!revision) {
          throw new AutomationHistoryClearError(
            "AUTOMATION_HISTORY_CLEAR_CANONICAL_MISMATCH",
          );
        }
...
async function clearAutomationHistory(
  ...
  const accepted = await runWithOriginExclusiveLock(
    AUTOMATION_SERVER_OPERATION_LOCK,
    () =>
      runWithOriginExclusiveLock(
...
function pendingPurgeRows(
  ...
  for (const value of values) {
    if (!value || typeof value !== "object" || (value as { kind?: unknown }).kind !== "purge_pending") {
      continue;
    }
  ...
  return rows.sort(
    (left, right) => left.createdAt - right.createdAt || left.operationId.localeCompare(right.operationId),
  );
}
...
export async function reconcilePendingAutomationHistoryPurges(
  ...
  for (const receipt of activeReceipts) {
    try {
      ...
      deferred += 1;
      logger.warn("[AutomationHistory] Pending purge remains deferred", {
        operationId: receipt.operationId,
...
export function forgetAutomationTransactions(
  ...
  return clearAutomationHistory(
    transactionIds,
    false,
    expectedOwnerUserId,
    deviceId,
  ) as Promise<{ purged: number; all: false }>;
}
