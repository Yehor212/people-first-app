...
const SAFE_STRING_VALUES: Readonly<Record<string, ReadonlySet<string>>> = {
  ...
  source: new Set(["boot", "capacitor", "cloud", "local", "native", "react", "storage", "sync", "tauri", "test", "web"]),
  state: new Set(["active", "cleared", "disabled", "enabled", "inactive", "set"]),
  status: new Set(["aborted", "active", "failed", "ok", "pending", "success", "timeout", "unavailable"]),
...
function safeStringMetadataValue(key: string, value: string): string {
  ...
  if (
    normalized === "action" &&
    /^(?:canonical|orb|webgl|webgpu)-[a-z0-9-]{1,64}$/.test(value)
  ) {
    return value;
  }
  ...
  if (normalized === "version" && /^\d{1,4}(?:\.\d{1,4}){0,3}(?:-[A-Za-z0-9.-]{1,24})?$/.test(value)) {
    return value;
  }
...
export function diagnosticCodeFrom(
  value: unknown,
