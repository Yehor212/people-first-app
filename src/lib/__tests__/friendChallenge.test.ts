...
vi.mock('@/lib/validation', () => ({
  generateSecureId: vi.fn((prefix: string) => `${prefix}_mock_${Math.random().toString(36).slice(2, 8)}`),
}));
...
describe('decodeInviteData', () => {
  ...
  it('returns null for valid JSON but missing required fields', () => {
    ...
    // Should return object with undefined fields (no validation on fields)
    // The function will still parse it, just with missing fields
    expect(result).not.toBeNull();
    expect(result?.habitName).toBeUndefined();
  });
});
