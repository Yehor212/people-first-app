...
describe('offlineQueueHandlers', () => {
  ...
  describe('initializeOfflineQueueHandlers', () => {
    it('registers handlers for all expected action types', () => {
      ...
      const registeredTypes = vi.mocked(offlineQueue.registerHandler).mock.calls.map(
        (call) => call[0]
      );
  ...
  describe('registered handler behavior', () => {
    ...
    function getHandler(actionType: string): (action: OfflineAction) => Promise<void> {
      ...
      const call = vi.mocked(offlineQueue.registerHandler).mock.calls.find(
        (c) => c[0] === actionType
      );
    ...
    it('TOGGLE_HABIT handler calls syncHabit with valid payload', async () => {
      const handler = getHandler('TOGGLE_HABIT');
      const habit = makeHabit({ id: 'habit-toggle' });
      await handler(makeAction('TOGGLE_HABIT', habit, habit.id));
  ...
  describe('queue helper functions', () => {
    ...
    it('queueFocusSessionSync enqueues with CREATE_FOCUS_SESSION', async () => {
      ...
      expect(offlineQueue.enqueue).toHaveBeenCalledWith('CREATE_FOCUS_SESSION', 'focus-q1', session);
    });
