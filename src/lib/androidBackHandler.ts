...
const TRANSIENT_VISUAL_BACK_SELECTORS = [
  '[role="menu"]',
];
...
function claimCurrentModalVisualLayer(): Element | null {
  const claimedLayers = new Set(
    backOwners.map((owner) => owner.visualElement).filter((element): element is Element => !!element),
  );
  return (
...
function closeRegisteredBackOwner(
  layer: BackOwnerLayer,
