...
export const habitSchema = z.object({
  ...
  schedule: z.object({
    mode: z.enum(['daily', 'flexiblePerPeriod', 'specificDays']),
    period: z.literal('week'),
    targetCount: z.number().int().min(1).max(7),
    dueDays: z.array(z.number().int().min(0).max(6)).optional(),
  }).optional(),
  ...
  quickEntryMode: z.enum(['completeTarget', 'incrementStep', 'limitCheck']).optional(),
