...
describe("useChallengeHandlers", () => {
  ...
  it("updateChallengeProgress computes stats and calls syncChallengeProgress", () => {
    const params = {
      ...
      safeFocusSessions: [
        { id: "f1", duration: 30, completedAt: 1, date: "2026-02-19" },
      ],
  ...
  it("perfect days calculation counts days where ALL habits are completed", () => {
    // Pin createdAt before test dates — Date.now()-30d drifts and eventually passes test dates
    const createdBefore = new Date("2026-01-01").getTime();
    const params = {
      ...defaultParams,
      safeHabits: [
        makeTestHabit({
  ...
  it("zen master days counts days with mood + all habits + focus + gratitude", () => {
    // Pin createdAt before test dates — same drift fix as perfectDaysCount test
    const createdBefore = new Date("2026-01-01").getTime();
    const params = {
      ...defaultParams,
      safeMoods: [
