...
describe("useCloudSyncEffects", () => {
  describe("native resume sync ownership", () => {
    it("lets the delta runtime own core sync when delta sync is enabled", () => {
      expect(shouldRunLegacyBackupSyncOnNativeResume(true)).toBe(false);
    });

    it("keeps legacy backup resume sync available when delta sync is disabled", () => {
      expect(shouldRunLegacyBackupSyncOnNativeResume(false)).toBe(true);
    });
  });
