...
vi.mock("@/lib/safeJson", () => ({
  safeJsonParse: vi.fn((_value: string, fallback: unknown) => fallback),
...
describe("T182 focus-reflection Back ownership", () => {
  it("has one Back owner that cancels without completing the pending session", async () => {
  ...
  it("uses the same no-complete cancel semantics for keyboard Escape", async () => {
    const onCompleteSession = vi.fn();
    backOwners.clear();
    render(<ReflectionHarness onCompleteSession={onCompleteSession} />);
