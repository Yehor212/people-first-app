...
import type { TimerState } from "@/types/focusTimerTypes";
import { DEFAULT_FOCUS_MINUTES, DEFAULT_BREAK_MINUTES } from "@/types/focusTimerTypes";
...
export function useFocusTimerConfig(savedState: TimerState | null) {
  ...
  const [focusMinutes, setFocusMinutes] = useState(
    savedState?.focusMinutes || DEFAULT_FOCUS_MINUTES
  );
  const [breakMinutes, setBreakMinutes] = useState(
    savedState?.breakMinutes || DEFAULT_BREAK_MINUTES
  );
  const [savedCustomFocus, setSavedCustomFocus] = useState(savedState?.focusMinutes || 30);
  const [savedCustomBreak, setSavedCustomBreak] = useState(
    savedState?.breakMinutes || DEFAULT_BREAK_MINUTES
  );
