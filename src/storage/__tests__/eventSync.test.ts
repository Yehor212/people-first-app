...
const mocks = vi.hoisted(() => ({
  ...
  automationTransactionsPut: vi.fn(),
  table: vi.fn(),
  transaction: vi.fn(),
  habitTableGet: vi.fn(),
  habitTablePut: vi.fn(),
...
vi.mock("@/storage/db", () => ({
  ...
  db: {
    ...
    automationTransactions: {
      put: mocks.automationTransactionsPut,
    },
    habits: {
...
describe("eventSync auth guards", () => {
  beforeEach(() => {
    ...
    mocks.journalEntriesIn.mockResolvedValue({ data: [], error: null });
  ...
  it("applies a connected planning setting with the exact authoritative revision", async () => {
    const ownerUserId = "11111111-1111-4111-8111-111111111111";
    mocks.localOwnerUserId = ownerUserId;
    mocks.getCurrentUserId.mockResolvedValue(ownerUserId);
    const schedule = [{
    ...
    const { hashAutomationValue } = await import("@/features/automation/canonicalJson");
    const stateHash = await hashAutomationValue(schedule);

    const applied = await applyDelta(
      [{
        id: "event-connected-planning-setting",
        seq: 15,
        entity_type: "setting",
    ...
    expect(mocks.automationTransactionsPut).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("fetches the current owner-RLS journal row for a contentless ordered event", async () => {
    mocks.journalEntriesIn.mockResolvedValue({
      data: [
        {
          id: "journal-contentless-1",
          user_id: "user-1",
          date: "2026-08-08",
          title: "Current private title",
          content: "Current private content",
          stickers: [],
          mood: null,
          tags: [],
          template_id: null,
    ...
    const applied = await applyDelta(
      [
        {
          id: "event-journal-contentless-1",
          seq: 20,
          entity_type: "journal",
          entity_id: "journal-contentless-1",
          op: "upsert",
    ...
    expect(applied).toBe(1);
    expect(mocks.journalEntriesIn).toHaveBeenCalledWith("id", ["journal-contentless-1"]);
    expect(mocks.journalTablePut).toHaveBeenCalledWith(
      expect.objectContaining({
        id: "journal-contentless-1",
        title: "Current private title",
