...
const OUTPUT_DIR = path.resolve("output/android21/t153-leaderboard-entry-row-reflow");
...
test.describe("leaderboard entry row reflow", () => {
  ...
  test("keeps rank, participant identity and streak values associated inside every row", async ({
    browser,
  }) => {
    ...
    for (const viewport of VIEWPORTS) {
      for (const locale of LOCALES) {
        ...
        await page.goto(
          `e2e/helpers/leaderboard-entry-row-reflow/index.html?lang=${locale}&theme=${theme}&scale=${viewport.scale}`,
          ...
          .poll(
            () =>
              card.evaluate((cardElement) =>
                Array.from(cardElement.children).every((row) => {
        ...
        const facts = await card.evaluate((cardElement) => {
          ...
          if (rows.length === 0) throw new Error("leaderboard rows not found");
          ...
          rows.forEach((row, rowIndex) => {
            ...
            while (textNode) {
              ...
              if (text.trim()) {
                ...
                if (rects.some((rect) => outsideInline(rect, rowRect) || outsideInline(rect, cardRect))) {
          ...
          return {
            ...
            cardOutsideViewport:
              cardRect.left < -tolerance || cardRect.right > document.documentElement.clientWidth + tolerance,
    ...
    for (const row of receipt) {
      ...
      expect(row.crownCount, `${label} first-rank crown`).toBe(1);
