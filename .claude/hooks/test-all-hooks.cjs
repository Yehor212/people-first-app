...
try {
  ...
  const pv2 = pv.validate(JSON.stringify({
    ...
    pre_mortem: 'Could fail because src/App.tsx:42 validation rules are misconfigured and .claude/hooks/preflight-validate.cjs:562 return logic is fragile — if regex wrong then 2 edge cases break. Pattern observed across validators. Most likely cause: insufficient test coverage.',
...
try {
  ...
  const pvr19b = pv.validate(JSON.stringify({
    ...
    checks_completed: 7,
    evidence: { read: ['src/App.tsx:1'], search: ['grep test'], assumed: [] },
    pre_mortem: 'Should pass because src/App.tsx:1 exists on disk and .claude/hooks/preflight-validate.cjs:200 checks file existence — if file missing then rule 19 blocks. Pattern observed: most likely valid path.',
