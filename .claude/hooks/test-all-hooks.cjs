...
try {
  ...
  const pv2 = pv.validate(JSON.stringify({
    ...
    pre_mortem: 'Could fail because src/App.tsx:42 validation rules are misconfigured — if regex wrong then 2 edge cases will break. Pattern observed across 3 similar validators. Root cause: most likely insufficient test coverage.',
...
try {
  ...
  const pvr19b = pv.validate(JSON.stringify({
    ...
    checks_completed: 7,
    evidence: { read: ['src/App.tsx:1'], search: ['grep test'], assumed: [] },
    pre_mortem: 'Should pass because src/App.tsx exists on disk — if file missing then rule 19 blocks. Pattern observed in evidence veracity checks. Root cause hypothesis: file path validation.',
