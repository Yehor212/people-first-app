...
const PREFLIGHT_PROTOCOL = [
  ...
  'META-LAYER TEMPLATE (for "find ALL problems" / full audit / deep analysis):',
  'MANDATORY: check ALL stack layers, not just current context. Anti-Pattern #16: Convenience Bias.',
  '  Layer 1 LOCAL: types(tsc), lint(eslint), tests(vitest), hardcoded-colors, as-any, silent-catch, i18n(8 langs)',
  '  Layer 2 BACKEND: Supabase MCP logs(api,auth,edge-function,postgres,storage,realtime), cron jobs, RLS policies, vault secrets, triggers',
  '  Layer 3 MOBILE: capacitor-platform(Android back,safe-areas,touch-44px), -webkit-backdrop-filter, motion-safe, haptics',
  '  Layer 4 EXTERNAL: Firebase(push tokens,analytics), npm-audit(security), npm-outdated(deps)',
  '  Layer 5 CI/CD: ci:preflight, ratchet:check, GitHub Actions, build size',
  'Skip a layer ONLY with explicit reason. "Was working on hooks" is NOT a valid reason.',
  '',
  ...
  '#16 Convenience Bias (checking only local/familiar systems, skipping backend/infra. Incident: missed 401s in Supabase for 2 days).',
